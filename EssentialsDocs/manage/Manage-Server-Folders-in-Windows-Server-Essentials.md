...
description: Сведения об управлении доступом к любым папкам сервера на сервере с помощью задач на вкладке "папки сервера" панели мониторинга.
ms.date: 10/03/2016
ms.topic: article
...
##  <a name="manage-server-folders-using-the-dashboard"></a><a name="BKMK_2"></a> Управление папками сервера с помощью панели мониторинга
...
|Просмотр свойств папки|Отображение свойств выбранной папки. Вы можете:<br /><br /> -Изменить имена папок, созданных пользователем.<br /><br /> -Изменить описание для выбранной папки.<br /><br /> — Просмотр размера папки.<br /><br /> — Открыть выбранную папку в проводнике.<br /><br /> — Укажите разрешения на доступ к учетной записи пользователя для выбранной папки.<br /><br /> -Скрыть выбранную папку из удаленных Веб-доступ и приложений веб-служб.<br /><br /> — Укажите квоту папки.|
...
##  <a name="manage-access-to-server-folders"></a><a name="BKMK_1"></a> Управление доступом к папкам сервера
...
|Company|Используется для хранения и доступа пользователей сети к документам, относящихся к вашей организации.|
...
-   [Просмотр и изменение свойств папки сервера](Manage-Server-Folders-in-Windows-Server-Essentials.md#BKMK_10)

###  <a name="hide-server-folders"></a><a name="BKMK_Hide"></a> Скрыть папки сервера
...
4.  В области **<\> задачи Серверфолдер** щелкните **Просмотр свойств папки**.

5.  В **\> свойствах<имя_папки** щелкните **общий доступ**, выберите **Скрыть эту папку от удаленного веб-доступ и приложений веб-службы**, а затем нажмите кнопку **Применить**.

###  <a name="set-permissions-to-server-folders"></a><a name="BKMK_Perms"></a> Задание разрешений для папок сервера
...
###  <a name="view-or-modify-server-folder-properties"></a><a name="BKMK_10"></a> Просмотр или изменение свойств папки сервера
...
5.  В **\> свойствах<имя_папки** на вкладке **Общие** просмотрите или измените имя и описание папки сервера.

    > [!NOTE]
    >  В Windows Server Essentials и Windows Server 2012 R2 с установленной ролью Windows Server Essentials можно также изменить квоту папки, которая выдает предупреждающее сообщение, когда Серверная папка достигнет указанного размера.

##  <a name="add-or-move-a-server-folder"></a><a name="BKMK_5"></a> Добавление или перемещение папки сервера
...
###  <a name="where-to-add-or-move-a-server-folder"></a><a name="BKMK_6"></a> Место добавления или перемещения папки сервера
...
-   Рядовой сервер под Windows Server Essentials или Windows Server 2012 R2 с установленной ролью Windows Server Essentials Experience
...
4. Для завершения работы мастера следуйте инструкциям на экране.
   ...
   >   -   Можно определить, какие папки сервера доступны на сайте удаленного веб-доступа. Дополнительные сведения см. в разделе [Управление доступом к папкам сервера](Manage-Server-Folders-in-Windows-Server-Essentials.md#BKMK_1).
...
##  <a name="add-a-missing-server-folder"></a><a name="BKMK_9"></a> Добавление отсутствующей папки сервера
 Когда сервер обнаруживает, что предопределенная серверная папка? Организация, пользователи, резервное копирование клиентских компьютеров, резервное копирование журнала файлов или перенаправление папок больше не являются общими (по какой-либо причине или другой), создается предупреждение, которое поможет пользователю устранить эту проблему. Рекомендуется попробовать восстановить папку из резервной копии сервера. Но если резервная копия сервера не была создана, выберите отсутствующую папку и нажмите кнопку **Заново создать отсутствующую папку**, чтобы повторно настроить расположение папки сервера.

> [!NOTE]
>  Только предварительно определенные папки? Компании, пользователи, резервное копирование клиентских компьютеров, резервное копирование журнала файлов или перенаправление папок? можно создать повторно. Пользовательские папки и папки сервера мультимедиа не могут быть созданы заново.

 После восстановления или воссоздания отсутствующей папки она больше не будет указана как **отсутствующая**.

 Сведения о восстановлении файлов из резервных копий сервера см. в разделе Дополнительные сведения о восстановлении файлов и папок статьи [Управление резервным копированием и восстановлением](Manage-Backup-and-Restore-in-Windows-Server-Essentials.md).

##  <a name="understand-shared-folders"></a><a name="BKMK_11"></a> Общие сведения об общих папках
...
##  <a name="understand-shadow-copies"></a><a name="BKMK_Shadow"></a> Общие сведения о теневых копиях
