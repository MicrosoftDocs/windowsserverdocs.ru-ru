...
description: Сведения о стандартных задачах резервного копирования для клиентских компьютеров, которые можно выполнить с помощью панели мониторинга Windows Server Essentials.
ms.date: 10/03/2016
ms.topic: article
...
3.  Состояние архивации отображается в разделе **Состояние архивации** диалогового окна **Свойства архивации**.
    ...
    -   **Резервное копирование отменено**: отображает дату и время начала резервного копирования, если вы или администратор сети отменили резервную копию.
...
4.  При прекращении выполнения резервного копирования состояние изменения в **BACKUP было отменено** с датой и временем начала резервного копирования. Если вместо этого отображается состояние **Успешно**, **Не завершено** или **Сбой**, последняя архивация уже завершилась.
...
5.  После запуска архивации кнопка меняется на **Остановить архивацию**. Если выполняется архивация и необходимо остановить ее, нажмите кнопку **Остановить архивацию** и затем кнопку **Да**. При прекращении выполнения резервного копирования состояние изменения в **BACKUP было отменено** с датой и временем начала резервного копирования.
