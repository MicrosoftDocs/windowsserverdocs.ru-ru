---
title: Понижение уровня и удаление исходного сервера из новой сети Windows Server Essentials
...
ms.locfileid: "97977279"
---
# <a name="demote-and-remove-the-source-server-from-the-new-windows-server-essentials-network"></a>Понижение уровня и удаление исходного сервера из новой сети Windows Server Essentials
...
1. [Удаление Exchange Server](#uninstall-exchange-server).

2. [Отключите принтеры, подключенные напрямую к исходному серверу](#disconnect-printers-directly-connected-to-the-source-server).

3. [Понижение уровня исходного сервера](#demote-the-source-server).

4. [Переместите роль DHCP-сервера на маршрутизатор](#move-the-dhcp-server-role-to-the-router).

5. [Удалите и заново настройте исходный сервер](#remove-and-re-purpose-the-source-server).

## <a name="uninstall-exchange-server"></a>Удаление Exchange Server

Необходимо удалить Exchange Server с исходного сервера перед понижением его уровня. При этом будут удалены все ссылки в доменных службах Active Directory (AD DS) для Exchange Server на исходном сервере. Для удаления Exchange Server необходим носитель Windows Small Business Server.

> [!IMPORTANT]
> Если вы добавляете учетные записи пользователей после перемещения почтовых ящиков на целевой сервер и перед удалением Exchange Server с исходного сервера, почтовые ящики добавляются на исходном сервере. Это сделано намеренно. Почтовые ящики необходимо переместить на целевой сервер для всех учетных записей пользователей, которые добавляются в течение этого периода времени. Повторите инструкции из раздела перемещение почтовых ящиков и параметров Exchange Server для миграции Windows Server Essentials перед удалением Exchange Server.

### <a name="to-uninstall-exchange-server-from-the-source-server"></a>Удаление Exchange Server с исходного сервера
...
3. В списке программ выберите **Windows Small Business Server**, а затем нажмите кнопку **изменить или удалить**.
...
## <a name="demote-the-source-server"></a>Понижение уровня исходного сервера
...
## <a name="move-the-dhcp-server-role-to-the-router"></a>Перемещение роли DHCP-сервера на маршрутизатор

Если на исходном сервер выполняется роль DHCP, для перемещения ее на маршрутизатор выполните следующие действия.

> [!NOTE]
> Если вы уже выполнили эту задачу до начала процесса миграции, перейдите к разделу [Удаление и повторное назначение исходного сервера](#remove-and-re-purpose-the-source-server).
...
## <a name="remove-and-re-purpose-the-source-server"></a>Удаление и повторное назначение исходного сервера
