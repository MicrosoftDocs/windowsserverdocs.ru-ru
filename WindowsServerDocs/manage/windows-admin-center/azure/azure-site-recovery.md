...
- [Ознакомьтесь с инструментом планирования ресурсов, чтобы оценить требования к успешной репликации и отработке отказа](/azure/site-recovery/hyper-v-site-walkthrough-capacity).
...
2. Перейдите к **Virtual Machines**разделу  >  **Инвентаризация**виртуальных машин.
3. Выберите любую виртуальную машину (это не обязательно должна быть виртуальная машина, которую необходимо защитить).
4. Выберите **больше**  >  **настроить защиту виртуальной машины**.
5. Войдите в учетную запись Azure.
6. Введите необходимые сведения:

   - **Подписка:** подписка Azure, которую вы хотите использовать для репликации виртуальных машин на этом узле.
   - **Расположение:** регион Azure, в котором должны быть созданы ресурсы ASR.
   - **Учетная запись хранения:** учетная запись хранения, в которой будут сохраняться реплицированные рабочие нагрузки виртуальных машин на этом узле.
   - **Хранилище:** выберите имя хранилища Azure Site Recovery для защищаемых виртуальных машин на этом узле.

7. Выберите **Настройка ASR**.
8. Дождитесь появления уведомления: **Настройка Site Recovery завершена.**

Может составлять до 10 минут. Можно отслеживать ход выполнения, перейдя в раздел **Уведомления** (значок звонка в верхнем правом углу).
...
## <a name="step-2-select-virtual-machines-to-protect"></a>Этап 2. Выбор виртуальных машин для защиты
...
3. Выберите **больше**  >  **защитить виртуальную машину**.
4. Ознакомьтесь с [требованиями к емкости для защиты виртуальной машины](/azure/site-recovery/site-recovery-capacity-planner).

    Если вы хотите использовать учетную запись хранения класса Premium, [создайте ее на портал Azure](/azure/storage/common/storage-premium-storage). Для создания стандартной учетной записи хранения используется параметр **Создать новую** на панели Windows Admin Center.
...
1. [Настройте сеть Azure](/azure/site-recovery/hyper-v-site-walkthrough-prepare-azure). Виртуальная машина с отработанным отказом будет подсоединена к этой виртуальной сети. Обратите внимание, что другие действия, указанные на странице по ссылке, будут выполнены автоматически платформой Windows Admin Center; необходимо только настроить сеть Azure.

2. [Запустите тестовую отработку отказа](/azure/site-recovery/hyper-v-site-walkthrough-test-failover).

## <a name="step-4-create-recovery-plans"></a>Этап 4. Создание планов восстановления

**План восстановления** — это функция Azure Site Recovery, позволяющая выполнять отработку отказа и восстановление всего приложения, включающего в себя коллекцию виртуальных машин. Несмотря на то что можно восстановить защищенные виртуальные машины по отдельности путем добавления в план восстановления виртуальных машин, образующих приложение, вы сможете выполнить отработку отказа всего приложения с помощью плана восстановления. Также можно использовать функцию тестовой отработки отказа плана восстановления для тестирования процесса восстановления приложения. План восстановления позволяет группировать виртуальные машины, задавать их порядок, согласно которому они будут загружаться во время отработки отказа, а также автоматизировать дополнительные действия, которые необходимо выполнить в рамках процесса восстановления. После обеспечения защиты виртуальных машин можно перейти в хранилище Azure Site Recovery на портале Azure и создать планы восстановления для этих виртуальных машин. Дополнительные [сведения о планах восстановления](/azure/site-recovery/site-recovery-create-recovery-plans).

## <a name="monitoring-replicated-vms-in-azure"></a>Мониторинг реплицированных виртуальных машин в Azure ##

Чтобы убедиться в отсутствии ошибок при регистрации сервера, перейдите в **портал Azure**  >  **все ресурсы**  >  **хранилище служб восстановления** (указанное на шаге 2) > **заданиях**  >  **Site Recovery заданий**.

Вы можете отслеживать репликацию виртуальной машины, перейдя к **Recovery Services Vault**  >  **реплицированным элементам**хранилища служб восстановления.

Чтобы просмотреть все серверы, зарегистрированные в хранилище, перейдите в **хранилище служб восстановления**  >  **Site Recovery инфраструктуры**  >  **узлы Hyper-v** (в разделе сайты Hyper-v).

## <a name="known-issue"></a>Известная проблема ##

Если при регистрации ASR в кластере узлу не удается установить ASR или зарегистрировать службу ASR, это значит, что ваши виртуальные машины могут быть не защищены. Убедитесь, что все узлы в кластере зарегистрированы в портал Azure, перейдя к заданиям **хранилища служб восстановления**  >  **Jobs**  >  **Site Recovery задания**.