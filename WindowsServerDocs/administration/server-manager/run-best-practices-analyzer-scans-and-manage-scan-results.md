...
-   [Принципы работы анализатора соответствия рекомендациям](#BKMK_how)

-   [Проверка ролей анализатором соответствия рекомендациям](#BKMK_BPAscan)

-   [Управление результатами проверки](#BKMK_manage)

## <a name="find-bpa"></a><a name=BKMK_find></a>найти анализатор соответствия рекомендациям
...
## <a name="how-bpa-works"></a><a name=BKMK_how></a>Принципы работы анализатора соответствия рекомендациям
...
|Этап после развертывания|Правила после развертывания применяются после того, как все необходимые для роли службы будут запущены, и роль начнет выполняться на предприятии.|
|Предварительные условия|Правила необходимых условий поясняют, какие параметры конфигурации, параметры политик и компоненты необходимы для роли, прежде чем анализатор будет применять конкретные правила из прочих категорий. Необходимое условие в результатах проверки указывает на неверный параметр, отсутствующую программу, неправильно включенную или отключенную политику, параметр раздела реестра или другие особенности конфигурации, из-за которых анализатору соответствия рекомендациям не удается применить одно или несколько правил во время проверки. Результат проверки выполнения необходимого условия не означает соответствие или несоответствие. Оно говорит лишь о том, что правило нельзя применить, и поэтому не входит в результаты проверки.|

## <a name="performing-best-practices-analyzer-scans-on-roles"></a><a name=BKMK_BPAscan></a>Проверка ролей анализатором соответствия рекомендациям
...
##### <a name="to-include-scan-results-by-using-the-gui"></a><a name=BKMK_gui></a>Включение результатов проверки при помощи графического интерфейса анализатора соответствия рекомендациям
