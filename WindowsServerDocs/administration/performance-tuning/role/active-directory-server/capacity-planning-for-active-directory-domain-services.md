...
Разница заключается в том, что при постоянном превышении порогового значения управления емкостью (одноразовое событие не является проблемой) Добавление емкости (т. е. Добавление в более или более быстрых процессорах) является решением или масштабированием службы на нескольких серверах. Пороговые значения предупреждений производительности указывают на то, что в настоящее время взаимодействие с клиентом низкий, и для решения этой проблемы требуются немедленные действия.

В качестве аналога: Управление емкостью заключается в предотвращении случайного аварии машины (защитная работа, обеспечение нормальной работы Brakes и т. д.), в то время как устранение неполадок производительности заключается в том, что политика, пожарный отдел и экстренные специалисты делают это после аварии. Это "Защитная" Active Directory ".
...
- Читатели прочитали и знакомы с [рекомендациями по настройке производительности для Windows Server 2012 R2](/previous-versions//dn529133(v=vs.85)).
- Платформа Windows Server является архитектурой на основе x64. Но даже если ваша среда Active Directory установлена на Windows Server 2003 x86 (теперь за пределами жизненного цикла поддержки) и имеет дерево сведений о каталоге (DIT) размером менее 1,5 ГБ и может легко храниться в памяти, рекомендации из этой статьи все еще применимы.
...
| Компонент | Предполагаемая память (пример) |
|--|--|
...
Первое и самое важное замечание — Оценка размера NTDS. dit и SYSVOL. Эти измерения приводят к определению размера выделенного дискового пространства и объема ОЗУ. Из-за нехватки (относительной) низкой стоимости этих компонентов математические вычисления не требуются. Сведения о том, как оценить это как для существующих, так и для новых сред, можно найти в статье [хранение данных](/previous-versions/windows/it-pro/windows-2000-server/cc961771(v=technet.10)) в ряде статей. В частности, см. следующие статьи:

- **Для существующих сред &ndash; ** Раздел "Включение ведения журнала дискового пространства, освобожденного путем дефрагментации" в статье [ограничения хранилища](/previous-versions/windows/it-pro/windows-2000-server/cc961769(v=technet.10)).
- **Для новых сред &ndash; ** Статья, озаглавленная [Оценка роста для Active Directory пользователей и](/previous-versions/windows/it-pro/windows-2000-server/cc961779(v=technet.10))подразделений.
...
| Модификатор для разрешения автономной дефрагментации | 2,1 ГБ |
...
> [!NOTE]
> Основное внимание уделяется чтению базы данных, так как это, как правило, самый ресурсоемкий компонент, та же логика может применяться для записи в файл журнала путем подстановки LogicalDisk ( *\<NTDS Log\>* ) \Авг Disk sec/Write и LogicalDisk ( *\<NTDS Log\>* ) \ операций записи в секунду):
>
> - Логический *\<NTDS\>* диск () \авг. чтение с диска (сек.) "указывает, достаточно ли размера текущего хранилища.  Если результаты приблизительно равны времени доступа к диску для типа диска, логический диск ( *\<NTDS\>* ) \ операций чтения/с является допустимой мерой.  Проверьте спецификации производителя для хранилища в серверной части, но допустимые диапазоны для *\<NTDS\>* дискового пространства \авг sec ():
...
> - Логический *\<NTDS\>* диск () \ операций чтения/с — это объем выполняемых операций ввода-вывода.
>   - Если логический *\<NTDS\>* диск () \авг для чтения с диска (сек.) находится в оптимальном диапазоне для серверного хранилища, LogicalDisk ( *\<NTDS\>* ) \ операций чтения/с можно использовать непосредственно для изменения размера хранилища.
>   - Если логический *\<NTDS\>* диск () \авг время чтения с диска (сек.) не находится в оптимальном диапазоне для внутреннего хранилища, требуется дополнительный ввод-вывод в соответствии со следующей формулой: (логический диск ( *\<NTDS\>* ) \Авг время чтения с диска (сек.)) &divide; (доступ к физическому носителю данных) ( &times; логический диск ( *\<NTDS\>* ) \авг.
...
- LogicalDisk ( *\<NTDS Database Drive\>* ) \ передач в секунду в течение пикового периода 15 минут
- Чтобы определить объем операций ввода-вывода, необходимый для хранилища, в котором превышена емкость базового хранилища:
  >*Необходимые операции ввода-вывода* : (логический *\<NTDS Database Drive\>* диск () \авг, чтение/с &divide; *\<Target Avg Disk sec/Read\>* ) &times; LogicalDisk ( *\<NTDS Database Drive\>* ) \ чтения/с

| Счетчик | Значение |
|--|--|
| Фактический логический *\<NTDS Database Drive\>* диск () \Авг диска sec/передает | .02 с (20 миллисекунд) |
| Целевой логический *\<NTDS Database Drive\>* диск () \Авг диска sec/передает | .01 с |
| Коэффициент изменения в доступных операциях ввода-вывода | 0,02 &divide; 0,01 = 2 |

| Имя значения | Значение |
|--|--|
| Логический диск ( *\<NTDS Database Drive\>* ) \ передач/с | 400 |
...
- Независимо от того, правильно ли работают приложения в окружении в инфраструктуре общих служб и обсуждаются в разделе "Отслеживание дорогостоящих и неэффективных поисков" статьи создание более эффективных приложений с поддержкой Active Directory Майкрософт или переход от вызовов SAM нижнего уровня к вызовам LDAP.

  В более крупных средах это важно, так как плохо написанные приложения могут изменчивости нагрузку на ЦП, "украсть" время ЦП от других приложений, искусственно устранить потребности в емкости и равномерно распределить нагрузку на контроллеры домена.
...
| Система | Использование с по умолчанию | Создать Лдапсрввеигхт | Оценка нового использования |
|--|--|--|--|
...
| Система | \\  % &nbsp; Служебная программа процессора сведений о процессорах (_Total)<br />Использование с по умолчанию | Создать Лдапсрввеигхт | Оценка нового использования |
|--|--|--|--|
...
| Система | Настроенное использование | Создать Лдапсрввеигхт | Оценка нового использования |
|--|--|--|--|
...
| Категория | Счетчик производительности | Интервал и выборка | Назначение | Предупреждение |
|--|--|--|--|--|
| Процессор | Служебная программа "информация о процессоре" (_Total) \\ % Processor | 60 мин | 40 % | 60 % |
...
| Сеть | Сетевой интерфейс ( \* ) отправлено \/с<p>Сетевой интерфейс ( \* ) получено \/с | 30 мин | 40 % | 60 % |
| Хранилище | Логический *\<NTDS Database Drive\>* диск () \Авг чтения с диска (сек)<p>LogicalDisk ( *\<NTDS Database Drive\>* ) \Авг запись на диск (сек.) | 60 мин | 10 мс | 15 мс |
| Службы AD | \*Время хранения семафора \авераже Netlogon () | 60 мин | 0 | 1 с |

## <a name="appendix-a-cpu-sizing-criteria"></a>Приложение а. критерии размера ЦП
...
Каждый поток является независимой задачей, так как каждый поток имеет собственный стек и инструкции. Так как AD DS является многопоточным и количество доступных потоков можно настроить с помощью функции [просмотра и установки политики LDAP в Active Directory с помощью Ntdsutil.exe](https://support.microsoft.com/kb/315071), она хорошо масштабируется по нескольким логическим процессорам.
...
Именно поэтому долгосрочные средние значения емкости в 40% позволяют головному отделению о ненормальных пиковых нагрузках, будь то транзитный (например, плохо закодированный запрос, выполняемый в течение нескольких минут) или аномальные всплески в общей нагрузке (утром первого дня после длинных выходных).
...
- Перевод [PERF_100NSEC_TIMER_INV](/previous-versions/windows/embedded/ms901169(v=msdn.10))
  - *B* = число периодов бездействия в 100-нс, потраченных на логический процессор. Изменение в переменной "*X*" в вычислении [PERF_100NSEC_TIMER_INV](/previous-versions/windows/embedded/ms901169(v=msdn.10))
  - *T* = общее число интервалов 100-НС в заданном диапазоне времени. Изменение в переменной "*Y*" в вычислении [PERF_100NSEC_TIMER_INV](/previous-versions/windows/embedded/ms901169(v=msdn.10)) .
...
## <a name="appendix-b-considerations-regarding-different-processor-speeds-and-the-effect-of-processor-power-management-on-processor-speeds"></a>Приложение б. вопросы, связанные с различными тактами процессора, а также воздействие на тактовую частоту управления питанием процессора
...
Чтобы настроить оценки для разных процессоров, они использовались для защиты, исключая другие узкие места, описанные выше, чтобы предположить, что удвоение скорости процессора вдвое превышает объем обработки, который может быть выполнен.  На сегодняшний день внутренняя архитектура процессоров различается между процессорами, что более безопасный способ оценить влияние использования разных процессоров, чем были взяты данные, — использовать SPECint_rate2006ный тест производительности, полученный в составе стандартного ознакомительного использования.
...
1. Чтобы определить модификатор, используйте следующее уравнение:
   >((*Значение оценки целевой платформы на ядро*) &times; (*МГц на ядро базовой платформы*)) &divide;((*Значение оценки по базовому плану на ядро*) &times; (*МГц на ядро целевой платформы*))

    Используя приведенный выше пример:
   >(35,83 &times; 2000) &divide; (33,75 &times; 2300) = 0,92
1. Умножьте предполагаемое количество процессоров на модификатор.  В приведенном выше примере, чтобы переключиться с процессора 2650-2630 на процессор с частотой + +, необходимо умножить вычисленные процессоры 11,25 &times; 0,92 = 10,35.
...
- **Жесткий диск —** На среднем жестком диске 10 000-RPM имеется время поиска в течение 7 мс (МС) и 3 МС времени доступа. Время поиска — это среднее время, необходимое для перемещения головки чтения и записи в расположение на Негнущийся. Время доступа — это среднее время, затрачиваемое на чтение или запись данных на диск, если головной элемент находится в правильном расположении. Таким же средним временем считывания уникального блока данных на уровне 10 000-RPM HD является поиск и доступ к общему объему приблизительно 10 мс (или .010 секунд) на блок данных.
  ...
  | Описание: | 7200 RPM 9ms, 4 мс доступ | 10 000 RPM 7ms, 3 МС доступ | 15 000 RPM 4 мс, 2 мс доступ |
  |--|--|--|--|
...
- **Шина PCI —** Это часто наподобный компонент. В этом примере это не будет узким местом. Однако при увеличении масштаба системы это может стать узким местом. Для справки 32-разрядная шина PCI, работающая на 33Mhz, может передавать данные 133 МБ/с. Ниже приведено уравнение.
  ...
  Шина PCI с частотой 66 64 бит может поддерживать теоретический максимум (64 бит &divide; 8 бит на байт &times; 66 МГц) = 528 МБ/с. Кроме того, любое другое устройство (например, сетевой адаптер, второй контроллер SCSI и т. д.) уменьшит пропускную способность, доступную при совместном доступе, и устройства будут конкурировать за ограниченные ресурсы.

После анализа компонентов этой подсистемы хранения диск является ограничивающим фактором объема операций ввода-вывода, которые могут быть запрошены, и, следовательно, объема данных, которые могут переносить систему. В частности, в сценарии AD DS это 100 случайных операций ввода-вывода в секунду с шагом в 8 КБ, а всего 800 КБ в секунду при доступе к базе данных Jet. Кроме того, максимальная пропускная способность для дискового массива, выделенного для файлов журнала, имеет следующие ограничения: 300 последовательных операций ввода-вывода в секунду с шагом в 8 КБ, а всего 2400 КБ (2,4 МБ) в секунду.
...
| Примечания | Анализ узкого места | Диск | Шина | Адаптер | Шина PCI |
|--|--|--|--|--|--|
| Это конфигурация контроллера домена после добавления второго диска. Конфигурация диска представляет собой узкое место в 800 КБ/с. | Добавить 1 диск (всего = 2)<p>Ввод-вывод является случайным<p>Размер блока 4 КБ<p>10 000 RPM HD | 200 I/OS всего<br />800 КБ/с. |  |  |  |
| После добавления 7 дисков конфигурация диска по-прежнему является узким местом в 3200 КБ/с. | **Добавление 7 дисков (всего = 8)**  <p>Ввод-вывод является случайным<p>Размер блока 4 КБ<p>10 000 RPM HD | 800 операций ввода-вывода в секунду.<br />3200 КБ/с |  |  |  |
| После изменения операций ввода-вывода на последовательный сетевой адаптер становится узким местом, так как он ограничен 1000 операций ввода/вывода. | Добавление 7 дисков (всего = 8)<p>**Ввод-вывод является последовательным**<p>Размер блока 4 КБ<p>10 000 RPM HD |  |  | 2400 операции ввода-вывода в секунду могут быть прочитаны и записаны на диск, а контроллер — только 1000. |  |
| После замены сетевого адаптера адаптером SCSI, поддерживающим 10 000 операций ввода-вывода в секунду, узкое место возвращается к конфигурации диска. | Добавление 7 дисков (всего = 8)<p>Ввод-вывод является случайным<p>Размер блока 4 КБ<p>10 000 RPM HD<p>**Обновление адаптера SCSI (теперь поддерживает 10 000 операций ввода-вывода)** | 800 операций ввода-вывода в секунду.<br />3 200 КБ/с |  |  |  |
| После увеличения размера блока до 32 КБ эта шина становится узким местом, так как поддерживает только 20 МБ/с. | Добавление 7 дисков (всего = 8)<p>Ввод-вывод является случайным<p>**Размер блока 32 КБ**<p>10 000 RPM HD |  | 800 операций ввода-вывода в секунду. 25 600 КБ/с (25 МБ/с) можно читать и записывать на диск.<p>Шина поддерживает только 20 МБ/с |  |  |
| После обновления шины и добавления дополнительных дисков диск остается узким местом. | **Добавление 13 дисков (всего = 14)**<p>Добавление второго адаптера SCSI с 14 дисками<p>Ввод-вывод является случайным<p>Размер блока 4 КБ<p>10 000 RPM HD<p>**Обновление до 320 МБ/с SCSI-шины** | 2800 операций ввода-вывода<p>11 200 КБ/с (10,9 МБ/с) |  |  |  |
| После изменения операций ввода-вывода на последовательный диск остается узким местом. | Добавление 13 дисков (всего = 14)<p>Добавление второго адаптера SCSI с 14 дисками<p>**Ввод-вывод является последовательным**<p>Размер блока 4 КБ<p>10 000 RPM HD<p>Обновление до 320 МБ/с SCSI-шины | 8 400 операций ввода-вывода<p>33 600 Кб\с<p>(32,8 Мб\с) |  |  |  |
| После добавления более быстрых жестких дисков диск остается узким местом. | Добавление 13 дисков (всего = 14)<p>Добавление второго адаптера SCSI с 14 дисками<p>Ввод-вывод является последовательным<p>Размер блока 4 КБ<p>**15 000 RPM HD**<p>Обновление до 320 МБ/с SCSI-шины | 14 000 операций ввода-вывода<p>56 000 КБ/с<p>(54,7 МБ/с) |  |  |  |
| После увеличения размера блока до 32 КБ шина PCI становится узким местом. | Добавление 13 дисков (всего = 14)<p>Добавление второго адаптера SCSI с 14 дисками<p>Ввод-вывод является последовательным<p>**Размер блока 32 КБ**<p>15 000 RPM HD<p>Обновление до 320 МБ/с SCSI-шины |  |  |  | 14 000 операций ввода-вывода<p>448 000 КБ/с<p>(437 МБ/с) — ограничение на число операций чтения и записи для шпинделя.<p>Шина PCI поддерживает теоретический максимум в 133 МБ/с (максимально эффективно это 75%). |
...
> *Чтение операций ввода-вывода* + 2 операций ввода-вывода на &times; *Write I/O*  =  *диск*
...
> *Максимальное число операций ввода-вывода в секунду на* &times; диск 2 вращения &times; [(*% операций чтения*  +  *% операций записи*) &divide; (*% операций чтения* + 2 &times; *% операций записи*)] = *всего операций ввода-вывода в секунду*

RAID 1 + 0, ведет себя точно так же, как и RAID 1, в отношении расходов на чтение и запись. Однако теперь операции ввода-вывода распределяются по каждому зеркальному набору. If

> *Максимальное число операций ввода-вывода в секунду на* &times; диск 2 вращения &times; [(*% операций чтения*  +  *% операций записи*) &divide; (*% операций чтения* + 2 &times; *% операций записи*)] = *Общее число операций ввода-вывода*

в наборе RAID 1, если кратность (*N*) наборов RAID 1 является полосковой, общее количество операций ввода-вывода, которое можно обработать, составляет N &times; операций ввода-вывода для каждого набора RAID 1:

> *N* &times; {*Максимальное число операций ввода-вывода в секунду на скорость* &times; 2 дисков &times; [(*% операций чтения*  +  *% операций записи*) &divide; (*% операций чтения* + 2 &times; *% операций записи*)]} = *всего операций ввода-вывода*
...
> *Чтение* ввода-вывода &times; *Write I/O*+ 4 операций ввода-вывода  =  *Total I/O*

Аналогичным образом в наборе RAID 1, когда отношение операций чтения к записи и число дисков известно, следующее уравнение может быть получено из приведенного выше уравнения, чтобы определить максимальное число операций ввода-вывода, поддерживаемое массивом (Обратите внимание, что общее количество шпинделей не включает "диск", потерянный на четность):

> *Операций ввода-вывода на* &times; диск (*Шпиндели* – 1) &times; [(*% Операций чтения*  +  *% операций записи*) &divide; (*% операций чтения* + 4 &times; *% операций записи*)] = *всего операций ввода-вывода в секунду*
...
В любой архитектурной модели несколько сток переводятся в основной сток. Если какая-либо из этих сток или точки соединения останавливается, то резервное копирование выполняется только на момент создания точки соединения. В сценарии хранилища это может быть перегруженный параметр (сценарий SAN/NAS/iSCSI), проблемы совместимости драйверов (неправильное сочетание встроенного по драйвера/HBA/storport.sys), резервное копирование, антивирусная или дефрагментация. Чтобы определить, достаточно ли много места в хранилище, необходимо измерить размер операций ввода-вывода в секунду и объем операций ввода в нем. Для каждого соединения добавьте их вместе, чтобы обеспечить адекватный "Диаметр канала".
...
Полезно понимать, почему эти рекомендации существуют, чтобы изменения в технологии хранения могли быть адаптированы. Эти рекомендации существуют по двум причинам. Первый — это изоляция операций ввода-вывода, поэтому проблемы производительности (т. е. разбиение на страницы) на уровне операционной системы не влияют на производительность базы данных и профилей ввода/вывода. Во-вторых, файлы журналов для AD DS (и большинства баз данных) являются последовательными, а жесткие диски и кэши на дисках имеют огромное преимущество при использовании с последовательным вводом-выводом по сравнению с более случайными схемами ввода-вывода операционной системы, а также почти чисто произвольными шаблонами ввода-вывода для диска базы данных AD DS. Изолируя последовательный ввод-вывод на отдельный физический диск, можно увеличить пропускную способность. Проблема, представленная в современных вариантах хранения, заключается в том, что основные предположения, лежащие в основе этих рекомендаций, перестают быть верными. Во многих сценариях виртуализированного хранилища, таких как iSCSI, SAN, NAS и файлы образов виртуальных дисков, базовый носитель хранилища совместно используется на нескольких узлах, что позволяет полностью отменять как "изоляцию операций ввода-вывода", так и "оптимизацию последовательного ввода/вывода". В действительности эти сценарии добавляют дополнительный уровень сложности в том случае, если другие узлы, обращающиеся к общему носителю, могут снизить скорость реагирования на контроллер домена.
