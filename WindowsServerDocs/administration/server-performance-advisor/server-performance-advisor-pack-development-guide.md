...
ms.date: 10/16/2017
ms.topic: article
ms.openlocfilehash: e99b12129d6f7cb4777e70f3fb40202b2501ae58
ms.sourcegitcommit: db4c35ebe56d561768d2a657da9e6d6a791457bd
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/04/2021
ms.locfileid: "101823998"
---
...
* **Метаданные XML** (ProvisionMetadata.xml)
    ...
    * Группа сборщиков данных [журналы и оповещения производительности (PLA)](/previous-versions/windows/desktop/pla/pla-portal)
...
В этом разделе описываются некоторые основные элементы, составляющие пакет Advisor, включая ProvisionMetadata.xml и атрибуты.

Ниже приведен пример заголовка для файла ProvisionMetadata.xml.
...
### <a name="defining-the-data-collector-set"></a><a href="" id="bkmk-definedatacollector"></a>Определение группы сборщиков данных
...
Чтобы получить параметр реестра, укажите полный путь к имени параметра: HKEY \_ Local \_ Machine \\ MyKey \\ значения MyValue

Чтобы получить все параметры в разделе реестра, укажите полный путь к разделу реестра: HKEY \_ Local \_ Machine \\ MyKey\\

Чтобы собрать все значения в разделе реестра и его подразделах (PLA рекурсивно собирает данные реестра), используйте две обратные косые черты для последнего разделителя пути: HKEY \_ Local \_ Machine \\ MyKey\\\\

Чтобы получить данные реестра с удаленного компьютера, включите имя компьютера в начало пути реестра: HKEY \_ Local \_ Machine \\ MyKey \\ значения MyValue
...
Можно добавить любой запрос WMI. Дополнительные сведения о написании запросов WMI см. в разделе [WQL (SQL для WMI)](/windows/win32/wmisdk/wql-sql-for-wmi). В следующем примере запрашиваются расположение файла подкачки:
...
Имя столбца | Тип данных SQL | Описание
--- | --- | ---
Идентификатор | Int, не NULL | >уникальный идентификатор запроса
query | Nvarchar (4000) NOT NULL | Исходная строка запроса в метаданных для инициализации
...
В предыдущем примере счетчик \\ физический диск (в \* \\ среднем) будет запрашиваться каждую секунду.

Может существовать два экземпляра: **\_ Total** и **0 C: D:**, а выходные данные могут выглядеть следующим образом:

TIMESTAMP | CategoryName | CounterName | Значение экземпляра _Total | Значение экземпляра 0 C: D:
---- | ---- | ---- | ---- | ----
13:45:52.630 | PhysicalDisk | Среднее время обращения к диску (с) | 0.00100008362473995 |0.00100008362473995
13:45:53.629 | PhysicalDisk | Среднее время обращения к диску (с) | 0.00280023414927187 | 0.00280023414927187
13:45:54.627 | PhysicalDisk | Среднее время обращения к диску (с) | 0.00385999853230048 | 0.00385999853230048
13:45:55.626 | PhysicalDisk | Среднее время обращения к диску (с) | 0.000933297607934224 | 0.000933297607934224
...
---- | ---- | ---- | ----
PhysicalDisk | _Total | Среднее время обращения к диску (с) | 0.00100008362473995
PhysicalDisk | 0 C: D: | Среднее время обращения к диску (с) | 0.00100008362473995
PhysicalDisk | _Total | Среднее время обращения к диску (с) | 0.00280023414927187
PhysicalDisk | 0 C: D: | Среднее время обращения к диску (с) | 0.00280023414927187
PhysicalDisk | _Total | Среднее время обращения к диску (с) | 0.00385999853230048
PhysicalDisk | 0 C: D: | Среднее время обращения к диску (с) | 0.00385999853230048
PhysicalDisk | _Total | Среднее время обращения к диску (с) | 0.000933297607934224
PhysicalDisk | 0 C: D: | Среднее время обращения к диску (с) | 0.000933297607934224
...
CategoryName | Nvarchar (200) NOT NULL | Имя категории
...
куерипас | FullPath | парентпас | FileName | Содержимое
----- | ----- | ----- | ----- | -----
% WINDIR% \...\applicationHost.config |C:\Windows<br>\...\applicationHost.config | C:\Windows<br>\... \конфиг | applicationHost. Confi хранится | 0x3C3F78
...
Диск | Свободный размер диска (ГБ) | Общий размер (ГБ)
---- | ---- | ----
0 | 100 | 500
1 | 20 | 320
...
### <a name="sections"></a><a href="" id="bkmk-ui-section"></a>Разделы
...
Дополнительные сведения об этих типах данных SQL см. в разделе [типы данных (Transact-SQL)](/sql/t-sql/data-types/data-types-transact-sql).

### <a name="single-value-groups"></a><a href="" id="bkmk-ui-svg"></a>Одиночные группы значений
...
### <a name="list-value-tables"></a><a href="" id="bkmk-ui-lvt"></a>список таблиц значений
...
CounterName | InstanceName | Среднее значение | SUM
--- | :---: | :---: | :---:
Загруженность процессора, % | _Total | 10 | 20
Загруженность процессора, % | CPU0 | 20 | 30
...
в предыдущем примере системные администраторы могут заинтересовать только те ключи, в которых значение InstanceName равно \_ Total или CPU1. Фильтр можно определить следующим образом:
...
* @value вывод с плавающей запятой
...
5.  Дождитесь завершения задачи. Если отчет создан успешно, вернитесь в среду SSMS и найдите последнюю задачу.
    ...
    Идентификатор | SessionId | адвисорипаккажеид | репортстатусид | ластупдатетиме | срешолдверсионид
    :---: | :---: | :---: | :---: | :---: | :---:
...
|                                                                 Регистр символов в стиле Pascal                                                                 |                       "Верблюжий" стиль                        |             Прописные буквы             |
|-----------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------|-----------------------------------|
| <ul><li>Имена в ProvisionMetadata.xml</li><li>Хранимые процедуры</li><li>Функции</li><li>Имена представлений</li><li>Имена временных таблиц</li></ul> | <ul><li>Имена параметров</li><li>Локальные переменные</li></ul> | Использовать для всех зарезервированных ключевых слов SQL |
...
* Чтобы завершить процесс при возникновении ошибки, рассмотрите возможность использования инструкции **Set \_ commit Abort on** .

* Всегда включайте основной номер версии в отображаемое имя пакета Advisor.

## <a name="advanced-topics"></a><a href="" id="bkmk-advancedtopics"></a>Дополнительные разделы
...
2. При наличии конфликтов слияния следуют следующие правила.
   ...
   2. Взять супер набор счетчиков производительности. Например, при использовании **процесса ( \* ) \\ % времени процессора** и **процесса ( \* ) \\ \* \\ процесс ( \* ) \\ \\*** возвращает больше данных, поэтому **процесс ( \* ) \\ % загруженности процессора** и **процесса ( \* ) \\ \\*** удаляется из объединенного набора сборщиков данных.
...
Чтобы найти значение **FriendlyName** , откройте редактор реестра и перейдите к параметру реестра, объединив **\_ \_ \\ \\ \\ Перечисление \\ CurrentControlSet локальной системы hKey** с каждой строкой в предыдущем примере. , например: **hKey \_ локальный \_ компьютер \\ System \\ CurrentControlSet \\ Enum \\ root \\ \* IPHTTPS \\ 0000**.
...
Для сборщика данных WMI каждый объект WMI имеет множество присоединенных атрибутов. Любой тип объекта WMI всегда имеет три атрибута: \_ \_ пространство имен, \_ \_ класс и \_ \_ релпас.

Чтобы определить сборщик данных, на который ссылаются другие сборщики данных, назначьте атрибут **Name** с уникальным ключом в ProvisionMetadata.xml. Этот ключ используется зависимыми собирающими данными для создания динамических данных.
...
### <a name="tooltips"></a><a href="" id="bkmk-tooltips"></a>Подсказки
...
--- | --- | ---
WMI | WMI: &lt; &gt; / &lt; поле WMICLASS&gt; | Инструментарий WMI: Win32_OperatingSystem/Каптион
Счетчик производительности | Перфкаунтер: &lt; CategoryName &gt; / &lt; имя_экземпляра&gt; | Перфкаунтер: процесс/% загруженности процессора
реестр | реестр: &lt; регистеркэй&gt; | реестр: Хклм\софтваре\микрософт<br>\\ASP.NET \\ рутвер
...
Трассировка событий Windows | ETW: &lt; provider/ &gt; (ключевые слова) | ETW: Трассировка ядра Windows (процесс, NET)
...
Здесь описано, как определить ETW в файле ProvisionMetadata.xml:
...
properties | Hex | Свойства (необязательно)
...
ProcessId | BigInt, не NULL | ИД процесса
 ThreadId | BigInt, не NULL | Идентификатор потока
...
Схему ETW можно создать, запустив tracerpt.exe в ETL-файле. Создается файл Schema. Man. Так как формат ETL-файла зависит от компьютера, следующий сценарий работает только в следующих ситуациях:
...
Консоль SPA относится к SpaConsole.exe, которая является центральной частью советника по производительности сервера. SPA не требуется запускать на тестируемом целевом сервере. Консоль SPA содержит все пользовательские интерфейсы для SPA, от настройки проекта до выполнения анализа и просмотра отчетов. По разработке, SPA является двусторонним приложением. Консоль SPA содержит слой пользовательского интерфейса и часть уровня бизнес-логики. Консоль SPA планирует и обрабатывает запросы на анализ производительности.
