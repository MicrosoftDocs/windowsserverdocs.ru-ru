...
ms.date: 10/16/2017
ms.topic: article
ms.openlocfilehash: a55c00dc8d1dfc52760e8667c3a5b459ac96eecd
ms.sourcegitcommit: db4c35ebe56d561768d2a657da9e6d6a791457bd
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/04/2021
ms.locfileid: "101823988"
---
...
* Предоставьте рекомендации на основе собираемых данных.
...
### <a name="what-s-new-in-spa-31"></a><a href="" id="what-s-new-in-spa-3-1-"></a>Что нового в SPA 3,1?
...
Пользовательские интерфейсы и взаимодействия консоли SPA создаются в составе SPAConsole.exe. Консоль можно использовать для создания базы данных, добавления или удаления пакетов Advisor, управления целевыми серверами, выполнения анализа производительности и просмотра отчетов о производительности.
...
SPA основан на Windows Presentation Foundation (WPF) 2,0, который входит в состав Microsoft .NET Framework 4, поэтому требуется платформа .NET Framework 4.
...
**Примечание** . SPA не включает SQL Server или платформа .NET Framework как часть пакета установки SPA. После установки Microsoft SQL Server 2008 R2 и платформа .NET Framework 4,0 рекомендуется запустить Центр обновления Windows перед установкой SPA.
...
### <a name="setting-up-spa"></a><a href="" id="bkmk-setupspa"></a>Настройка SPA
...
* Если целевой сервер не присоединен к домену, включите следующий параметр реестра: **HKLM \\ Software \\ Microsoft \\ Windows \\ CurrentVersion \\ политики \\ System \\ LocalAccountTokenFilterPolicy**.

**Примечание** . По умолчанию SPA записывает журналы диагностики в папку, где находится SpaConsole.exe. Если параметр SPA установлен в папке Program Files, SPA может записывать только тот журнал, когда SpaConsole.exe запускается от имени администратора.
...
1.  Запустите SpaConsole.exe. Консоль переходит в отключенный режим, в котором SPA не подключена ни к одной базе данных, а главное окно — пустым.
...
3.  Щелкните **Далее**. На странице **Создание базы данных проекта** предлагается указать имя экземпляра Microsoft SQL Server, в котором нужно создать базу данных. Например, если он находится на том же компьютере, что и консоль, можно использовать **localhost \\ &lt; &gt; имя SQL Server**.
...
4.  Укажите имя базы данных. \_В качестве допустимых символов для имени базы данных допускаются только буквы, цифры и символы подчеркивания (). Разумным предложением для имени базы данных SPA будет **Spa**. Если ввести недопустимое имя, появится красный значок ошибки. Связанная подсказка выдает причину ошибки проверки.
...
## <a name="viewing-reports"></a><a href="" id="bkmk-viewingreports"></a>Просмотр отчетов
...
## <a name="managing-advisor-packs"></a><a href="" id="bkmk-manageadvisorpacks"></a>Управление пакетами Advisor
...
Новые пакеты Advisor могут выпускаться корпорацией Майкрософт или разработчиками сторонних разработчиков. Пакет Advisor включает в себя provisionMetaData.xml и набор скриптов SQL, описывающих логику.

**Предоставление нового пакета Advisor**

1.  Скопируйте все содержимое пакета Advisor в каталог *% спарут%* \\ APS.
...
Перед использованием этих командлетов рекомендуется закрыть приложение SPAConsole.exe.
...
| ------ | ------- | ------ |
| Start-SpaAnalysis | **-ServerName** Имя целевого сервера.<br>**-Адвисорпаккнаме** Полное имя пакета Advisor, добавляемого в очередь на сервере. Если запланировано одновременное выполнение нескольких пакетов, значение параметра должно быть отформатировано как AP1name, AP2name.<br>**— Длительность** Длительность сбора данных.<br>**Учетные данные** Учетные данные пользователя для учетной записи, выполняющей сбор данных на целевом сервере.<br>**-SqlInstanceName** Имя экземпляра SQL Server.<br>**-Склдатабасенаме** Имя базы данных проекта SPA. | Запускает сеанс сбора данных SPA на указанном сервере. |
| Stop-SpaAnalysis | **-SqlInstanceName** Имя экземпляра SQL Server.<br>**-Склдатабасенаме** Имя базы данных проекта SPA.<br>**-ServerName** Имя целевого сервера. | Пытается прерывать выполнение сеанса SPA. Если сеанс уже завершен, он возвращается без выполнения каких-либо действий. |
| Get-SpaServer | **-SqlInstanceName** Имя экземпляра SQL Server.<br>**-Склдатабасенаме** Имя базы данных проекта SPA. | Возвращает список серверов в базе данных. Он возвращает список объектов, включая следующие свойства: Name, Status, Общая папка и замечание. |
| Get-SpaAdvisorPacks | **-SqlInstanceName** Имя экземпляра SQL Server<br>**-Склдатабасенаме** Имя базы данных проекта SPA | Возвращает список пакетов Advisor в базе данных. Он возвращает список объектов, включая следующие свойства: Name, DisplayName, Author и Version. |

Windows PowerShell предоставляет возможность передавать учетные данные через зашифрованные файлы для включения сценариев автоматизации. Дополнительные сведения об использовании зашифрованных файлов для передачи учетных данных командлету см. в разделе [Создание сценариев Windows PowerShell, которые принимают учетные данные](/previous-versions/technet-magazine/ff714574(v=msdn.10)).
...
Данные отчета SPA можно извлечь с помощью SQL, чтобы создать пользовательские отчеты, не предоставляемые в SPAConsole.exe.
...
SPA предоставляет базовые функции ведения журнала. Он позволяет записывать только журналы в файл журнала, который находится в той же папке, что и SPAConsole.exe. Учетная запись пользователя, которая запускает консоль SPA, должна иметь разрешение на запись в папку, в которой была установлена SPA, чтобы гарантировать возможность записи журналов в файл журнала.
...
По умолчанию уровень ведения журнала — предупреждение. Это означает, что SPA регистрирует только ошибки и исключения, происходящие в SPA. Уровень журнала можно изменить, отредактировав файл **SpaConsole.exe.config** в той же папке, где находится SpaConsole.exe. Все журналы записываются в файл log.txt в той же папке.
...
### <a name="backup-and-restore-the-database"></a>Архивация сайта Configuration Manager
...
### <a name="spaconsoleexe-does-not-start-or-write-log-file"></a>SPAConsole.exe не запускается или не записывает файл журнала

При первой попытке запуска SPAConsole.exe, если платформа .NET Framework не установлен, приложение не запускается или не записывает файл журнала. Перед запуском SPA убедитесь, что платформа compatible.NET установлена и работает правильно.
...
### <a name="running-spa-against-the-console"></a><a href="" id="running-spa-against-the-console-"></a>Запуск SPA для консоли
...
SPA защищает целостность каждого отчета, успешно сформированного с помощью SPA. в то же время SPA не гарантирует, что все задачи анализа, поставленные в очередь, завершаются. Если вы видите непротиворечивые изменения состояния для сеансов анализа производительности или ошибок, которые заявляют, что система не может найти журналы производительности, созданные группой сборщиков данных, вероятно, это вызвано несколькими экземплярами консоли SPA, работающими в одной базе данных проекта SPA.
...
### <a name="spaconsoleexe-recurring-collection-is-disrupted"></a>SPAConsole.exe повторяющаяся коллекция прервана

При запуске SPAConsole.exe и использовании повторяющегося сбора данных (например, ежечасная коллекция) сервер, на котором работает SPAConsole.exe, не должен находиться в режиме энергосбережения, чтобы его можно было приостановить. SPA не проверяет политику энергосбережения. Это приостановленное действие может привести к нарушению регулярного сбора повторяющихся данных.
...
Если результаты сборщика данных содержат неполные данные, отличные от потерь трассировки ETW, и разработчик пакета Advisor добавил поддержку уведомления о потере событий ETW, информационная панель отображается поверх одного отчета, чтобы уведомить пользователя о потенциально непротиворечивом отчете, вызванном потерей данных. подробные сведения о потери данных можно найти в файле log.txt.
...
* **Spa SpaConsole.exe консоли** , которая является центральной частью Spa. SPA не требуется запускать на тестируемом целевом сервере. Консоль SPA содержит все пользовательские интерфейсы для SPA, от настройки проекта до выполнения анализа и просмотра отчетов. По разработке, SPA является двусторонним приложением. Консоль SPA содержит слой пользовательского интерфейса и часть уровня бизнес-логики. Консоль SPA планирует и обрабатывает запросы на анализ производительности.
