...
При первой попытке запуска SPAConsole.exe, если .NET Framework не установлен, приложение не запускается или не записывает файл журнала. Перед запуском SPA убедитесь, что платформа compatible.NET установлена и работает правильно.
