...
- **SchTasks.exe** выполняет те же операции, что и **запланированные задания** на **панели управления**. Эти инструменты можно использовать совместно и взаимозаменяемы.
- **Команда Schtasks** заменяет **At.exe**, средство, входящее в предыдущие версии Windows. Хотя **At.exe** по-прежнему входит в семейство Windows Server 2003, рекомендуется использовать команду **schtasks** в качестве средства планирования задач командной строки.
...
- Чтобы проверить, выполнялась ли запланированная задача, или узнать, почему не выполнялось запланированное задание, см. журнал транзакций службы планировщик задач, *системный_корневой_каталог*\SchedLgU.txt. В этом журнале записываются попытки запуска, инициированные всеми инструментами, которые используют службу, включая **запланированные задачи** и **SchTasks.exe**.
- В редких случаях файлы задач повреждаются. Поврежденные задачи не выполняются. При попытке выполнить операцию с поврежденными задачами **SchTasks.exe** выводит следующее сообщение об ошибке:
  ```
  ERROR: The data is invalid.
  ```
  Восстановить поврежденные задачи невозможно. Чтобы восстановить функции планирования задач в системе, используйте **SchTasks.exe** или **запланированные задачи** для удаления задач из системы и их повторного планирования.
...
##### <a name="s-computer"></a>ключ \<Computer>

Планирует задачу на указанном удаленном компьютере. Введите имя или IP-адрес удаленного компьютера (с символами обратной косой черты или без них). По умолчанию это локальный компьютер. Параметры **/u** и **/p** допустимы только при использовании параметра **/s**.

##### <a name="u-domainuser"></a>ть\<Domain>\]<User>
...
##### <a name="ru-domainuser--system"></a>/ru {[ \<Domain> \] <User> | Системой
...
|       Значение        |                                                    Описание                                                    |
|--------------------|-------------------------------------------------------------------------------------------------------------------|
| [\<Domain>\]<User> |                                       Указывает альтернативную учетную запись пользователя.                                        |
...
Предоставляет пароль для учетной записи пользователя, указанной в параметре **/ru** . Если этот параметр не задан при указании учетной записи пользователя, **SchTasks.exe** запрашивает пароль и скрывает введенный текст.

Не используйте параметр **/RP** для задач, запускаемых с учетными данными системной учетной записи (**система/ru**). Учетная запись System не имеет пароля, а **SchTasks.exe** не запрашивает его.

##### <a name="mo-modifier"></a>месяц \<Modifier>
...
|-------------|---------------|-----------|
|MINUTE|1 - 1439|Задача выполняется раз в \<N> минуту.|
|Каждый час|1 - 23|Задача выполняется каждый \<N> час.|
|ЕЖЕДНЕВНО|1 - 365|Задача выполняется каждый \<N> день.|
|Запуск|1 - 52|Задача выполняется каждую \<N> неделю.|
...
|Семейный|1 - 12|Задача выполняется каждый \<N> месяц.|
|Семейный|ластдай|Задача выполняется в последний день месяца.|
|Семейный|ПЕРВЫЙ, ВТОРОЙ, ТРЕТИЙ, ЧЕТВЕРТЫЙ, ПОСЛЕДНИЙ|Используйте с **/d** \<Day> параметром/d для выполнения задачи в определенной неделе и дне. Например, в третью среду месяца.|
...
|    Семейный    |          Нет или {1-12}          |          1 - 31          | Необязательный параметр и допустимый только без модификатора (**/mo**) (конкретный график даты) или, если параметру **/mo** присвоено значение 1-12 ( \<N> Расписание каждого месяца). Значение по умолчанию — день 1 (первый день месяца). |
...
Указывает время запуска задачи (при каждом запуске) в \<HH:MM> 24-часовом формате. Значение по умолчанию — текущее время на локальном компьютере. Параметр **/St** действителен с почасовой, ежечасной, ежедневной, еженедельно, ежемесячно и раз в расписаниях. Оно требуется для ОДНОКРАТНОго расписания.
...
Указывает время суток, в течение которого расписание выполнения задачи будет заканчиваться \<HH:MM> 24-часовым форматом. После указанного времени окончания программа **schtasks** не запускает задачу повторно, пока не будет повторяться время запуска. По умолчанию расписания задач не имеют времени окончания. Этот параметр является необязательным и действителен только с поминутным или ЕЖЕЧАСным расписанием.

Пример см. в следующих статьях:
-   Чтобы запланировать задание, которое выполняется каждые 100 минут в нерабочее время в разделе, **чтобы запланировать задание, которое выполняется каждые** \<N> **минут** .

##### <a name="du-duration"></a>#b3 \<Duration>
...
Пример см. в следующих статьях:
-   Чтобы запланировать выполнение задачи, выполняемой каждые 3 часа, в течение 10 часов в разделе, **чтобы запланировать задание, которое выполняется каждый** \<N> **час** .
...
| \<DD>/<MM>/<YYYY> |       Используйте в качестве формата для первых дней, например **болгарского** и **нидерландского (Нидерланды)**.        |
| \<YYYY>/<MM>/<DD> |          Используйте для форматов с первым годом, например **шведского** и **французского (Канада)**.          |

#a1 \<EndDate>

Указывает дату окончания срока действия расписания. Этот параметр является необязательным. Оно недопустимо в расписании ОДНОКРАТНОго входа, запуска, OnStart или OnStart. По умолчанию расписания не имеют даты окончания.
...
Эта команда также использует параметр **/s** для указания удаленного компьютера, а параметр **/u** — для выполнения команды с разрешениями учетной записи администратора пользователя. Поскольку параметр **/p** опущен, **SchTasks.exe** запрашивает у пользователя пароль учетной записи администратора.
...
Следующая команда планирует запуск программы AdminCheck.exe на общедоступном компьютере каждую пятницу в 4:00 утра, но только в том случае, если администратор компьютера вошел в систему.
...
```
В ответе **SchTasks.exe** отображает информационное сообщение и сообщение об успешном выполнении. Он не запрашивает пароль.
```
...
|  ть\<Domain>\]<User>  |                                                                                                                                                                 Выполняет эту команду с разрешениями указанной учетной записи пользователя. По умолчанию заданы разрешения текущего пользователя локального компьютера. Указанная учетная запись пользователя должна быть членом группы "Администраторы" на удаленном компьютере. Параметры **/u** и **/p** допустимы только для изменения задачи на удаленном компьютере (**/s**).                                                                                                                                                                  |
|     /p \<Password>      |                                                                                                                                                                                              Указывает пароль учетной записи пользователя, указанной в параметре **/u** . Если используется параметр **/u** , но пропущен параметр **/p** или аргумент password, **команда Schtasks** запрашивает пароль.</br>Параметры **/u** и **/p** допустимы только при использовании параметра **/s**.                                                                                                                                                                                               |
| /ru {[\<Domain>\]<User> |                                                                                                                                                                                                                                                                                                                                       Системой                                                                                                                                                                                                                                                                                                                                       |
...
|     /TR \<TaskRun>      |                                                                                                                                                                                  Изменяет программу, выполняемую задачей. Введите полный путь и имя файла исполняемого файла, файла скрипта или пакетного файла. Если опустить путь, программа **schtasks** предполагает, что файл находится в \<systemroot> каталоге выполнять. Указанная программа заменяет исходную программу, выполняемую задачей.                                                                                                                                                                                  |
...
|     /RI \<Interval>     |                                                                                                                                                                                                                                                                           Указывает интервал повторения для запланированной задачи в минутах. Допустимый диапазон — 1-599940 (599940 минут = 9999 часов).                                                                                                                                                                                                                                                                            |
...
|     #b3 \<Duration>     |                                                                                                                                                                                                                                                                                                     Задает Закрытие задачи в \<EndTime> или <Duration> , если она указана.                                                                                                                                                                                                                                                                                                      |
...
Следующая команда изменяет программу, с которой запускается задача проверки на вирусы, с VirusCheck.exe на VirusCheck2.exe. Эта команда использует параметр **/TN** для определения задачи и параметр **/tr** , чтобы указать новую программу для задачи. (Изменить имя задачи нельзя.)
```
schtasks /change /tn Virus Check /tr C:\VirusCheck2.exe
```
В ответе **SchTasks.exe** отображает следующее сообщение об успешном выполнении:
```
SUCCESS: The parameters of the scheduled task Virus Check have been changed.
```
В результате выполнения этой команды задача проверки на вирусы теперь запускается VirusCheck2.exe.
...
```
В ответ **SchTasks.exe** запрашивает пароль для учетной записи пользователя. Он скрывает введенный текст, поэтому пароль не отображается.
```
...
|         Термин          |                                                                                                                                                                 Определение                                                                                                                                                                  |
|-----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    /TN \<TaskName>    |                                                                                                                                                       Обязательный элемент. Определяет задачу.                                                                                                                                                        |
|    ключ \<Computer>     |                                                                                                           Указывает имя или IP-адрес удаленного компьютера (с обратными косыми чертами или без них). По умолчанию это локальный компьютер.                                                                                                           |
...
```
В ответ **SchTasks.exe** запускает сценарий, связанный с задачей, и отображает следующее сообщение:
```
...
```
Чтобы найти причину ошибки, просмотрите журнал транзакций "запланированные задачи" C:\Windows\SchedLgU.txt в SVR01. В этом случае в журнале появится следующая запись:
```
...
```
После выполнения команды **Change** команда **Run** повторяется. На этот раз программа Update.exe запускается и **SchTasks.exe** выводит следующее сообщение:
```
...
|         Термин          |                                                                                                                                                               Определение                                                                                                                                                                |
|-----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    /TN \<TaskName>    |                                                                                                                                         Обязательный элемент. Определяет задачу, которая запустила программу.                                                                                                                                         |
|    ключ \<Computer>     |                                                                                                                        Указывает имя или IP-адрес удаленного компьютера. По умолчанию это локальный компьютер.                                                                                                                        |
| ть\<Domain>\]<User> | Выполняет эту команду с разрешениями указанной учетной записи пользователя. По умолчанию команда выполняется с разрешениями текущего пользователя локального компьютера. Указанная учетная запись пользователя должна быть членом группы "Администраторы" на удаленном компьютере. Параметры **/u** и **/p** допустимы только при использовании параметра **/s**. |
|    /p \<Password>     |                        Указывает пароль учетной записи пользователя, указанной в параметре **/u** . Если используется параметр **/u** , но пропущен параметр **/p** или аргумент password, **команда Schtasks** запрашивает пароль.</br>Параметры **/u** и **/p** допустимы только при использовании параметра **/s**.                         |
|          /?           |                                                                                                                                                             Вывод справки.                                                                                                                                                              |
...
**SchTasks.exe** завершает только экземпляры программы, запущенной запланированной задачей. Для прекращения работы других процессов используйте средство TaskKill. Дополнительные сведения см. в разделе [taskkill](taskkill.md).
...
```
В ответ **SchTasks.exe** останавливает экземпляр Notepad.exe, запущенный задачей, и отображает следующее сообщение об успешном выполнении:
```
...
```
В ответ **SchTasks.exe** останавливает экземпляр Internet Explorer, запущенный задачей, и отображает следующее сообщение об успешном выполнении:
```
...
```
В ответ **SchTasks.exe** отображает следующее сообщение с подтверждением. Чтобы удалить задачу, нажмите клавишу Y<strong>.</strong> Чтобы отменить команду, введите **n**:
```
...
```
В ответе **SchTasks.exe** отображает следующие сообщения об успешном выполнении, указывающие, что удаляется только запланированная задача секурескрипт.
...
|      #b1 \<format>    |  Задает формат выходных данных. Допустимые значения: TABLE, LIST и CSV.                                                                                                                                 |
...
|    ключ \<Computer>     |                                                                                                           Указывает имя или IP-адрес удаленного компьютера (с обратными косыми чертами или без них). По умолчанию это локальный компьютер.                                                                                                           |
| ть\<Domain>\]<User> | Выполняет эту команду с разрешениями указанной учетной записи пользователя. По умолчанию команда выполняется с разрешениями текущего пользователя локального компьютера.</br>Указанная учетная запись пользователя должна быть членом группы "Администраторы" на удаленном компьютере. Параметры **/u** и **/p** допустимы только при использовании параметра **/s**. |
|    /p \<Password>     |                                        Указывает пароль учетной записи пользователя, указанной в параметре **/u** . Если используется параметр **/u**, но параметр **/p** или аргумент password не задан, **команда Schtasks** запрашивает пароль.</br>Параметры **/u** и **/p** допустимы только при использовании параметра **/s**.                                         |
|          /?           |                                                                                                                                                    Отображение справки в командной строке.                                                                                                                                                     |
...
schtasks /query
```
В ответе **SchTasks.exe** отображает задачи в формате простого таблицы по умолчанию, как показано в следующей таблице.
```
...
В ответе **SchTasks.exe** отображает подробный список свойств для всех задач. На следующем экране показан список задач для задачи, запланированной на выполнение в 4:00 утра. в последнюю пятницу каждого месяца:
```
HostName: RESKIT01
...
```
В ответ **SchTasks.exe** добавляет задачи, запланированные для компьютера Reskit16, в файл p0102.csv на локальном компьютере SVR01.
