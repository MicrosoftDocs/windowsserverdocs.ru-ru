...
manager: mtillman
ms.date: 10/16/2017
ms.openlocfilehash: 416c6ffcc587589f1c55c577294eb8fc3a3ad6e6
ms.sourcegitcommit: db4c35ebe56d561768d2a657da9e6d6a791457bd
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/04/2021
ms.locfileid: "101818598"
---
...
| /logFilePath `[<path+logfilename>]` | Указывает путь к файлу DNS. log. Путь по умолчанию: `%systemroot%\System32\Dns\Dns.log`. Можно указать другой путь, используя формат `path+logfilename` . |
...
| /секуресекондариес `<zonename>` | Определяет, какие серверы-получатели могут получить обновления зоны с сервера-источника для этой зоны. |
...
Проверяет, определяет ли IP-адрес действующий DNS-сервер или может ли DNS-сервер выполнять роль сервера пересылки, сервера корневых подсказок или основного сервера для определенной зоны.
...
- Команда **зонерефреш** принудительно выполняет проверку номера версии в записи ресурса SOA сервера-источника. Если номер версии на основном сервере выше номера версии сервера-получателя, инициируется зонная отправка, которая обновляет сервер-получатель. Если номер версии совпадает, перенос зоны не выполняется.
...
Сбрасывает IP-адреса основного сервера, который предоставляет сведения о переносе зоны в дополнительную зону.
...
| `<IPaddress>` | IP-адреса основных серверов вторичной зоны. |
...
Указывает список IP-адресов вторичных серверов, на которые отвечает сервер-источник при запросе на зонную пересылку.
...
| /секурелист | Указывает, что передачи зоны предоставляются только списку серверов. За этим параметром должен следовать IP-адрес или адреса, используемые основным сервером. |
| `<securityIPaddresses>` | Список IP-адресов, которые получают передачи зоны с сервера-источника. Этот параметр используется только с параметром **/секурелист** . |
...
| /нотифилист | Указывает, что уведомления об изменениях отправляются только в список серверов. За командой должен следовать IP-адрес или адреса, используемые основным сервером. |
...
- Используйте команду **зонересетсекондариес** на сервере-источнике, чтобы указать, как она реагирует на запросы на зонную пересылку с серверов-получателей.
