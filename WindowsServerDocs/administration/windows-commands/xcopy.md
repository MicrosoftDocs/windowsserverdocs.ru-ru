...
|Параметр|Описание|
|---------|-----------|
|\<Source>|Обязательный. Указывает расположение и имена файлов, которые необходимо скопировать. Этот параметр должен содержать либо диск, либо путь.|
...
В предыдущем примере это конкретное значение исходного параметра **. \\ TOC \* . yml** копирует те же три файла, даже если их **два символа \\ пути были удалены.** Однако файлы не копируются, если шаблон звездочки был удален из параметра source, что делает его простым **. \\ TOC. yml**.
