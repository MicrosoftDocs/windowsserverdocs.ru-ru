...
| /j | Копирует с помощью небуферизованного ввода-вывода (рекомендуется для больших файлов). |
...
| /нодкопи | Не копирует сведения о каталоге (/дкопи по умолчанию **: Da** ). |
| /нуффлоад | Копирует файлы без использования механизма разгрузки копий Windows. |
| /compress | Запрашивает сетевое сжатие при передаче файла, если это применимо. |
