...
|         Изменение имени компьютера в рабочей группе         |                                                                                                                                                                **Netdom ренамекомпутер \<currentcomputername\> /newname.:\<newcomputername\>** <br>Перезагрузите компьютер.                                                                                                                                                                 |
|                Запрет управления файлом подкачки                 |                                                                                                                                                                        **WMIC ComputerSystem, где name = " \<computername\> " Set аутоматикманажедпажефиле = false**                                                                                                                                                                         |
|                   Конфигурация файла подкачки                   |                                                            **WMIC пажефилесет WHERE Name = " \<path/filename\> " Set инитиалсизе = \<initialsize\> , MaximumSize =\<maxsize\>** <br>Где *путь/имя* файла — это путь к файлу подкачки и его имя, *инитиалсизе* — это начальный размер файла подкачки в байтах, а параметр *MAXSIZE* — максимальный размер страничного файла в байтах.                                                             |
...
|Задача|Get-Help|
|----|-------|
|Настройка сервера для использования прокси-сервера|**Netsh WinHTTP Set Proxy \<servername\> :\<port number\>** <br>**Примечание.** Установка основных серверных компонентов не может получить доступ к Интернету через прокси-сервер, для которого требуется пароль, чтобы разрешить подключения.|
|Настройка сервера для обхода прокси-сервера для адресов Интернета|**Netsh WinHTTP Set Proxy \<servername\> : \<port number\> обход-List = " \<local\> "**|
...
|                         Удаление обновления                          |                                 **расширение/f: \* \<update\> . msu c:\test** <br>Перейдите в каталог c:\test\ и откройте файл \<update\>.xml in в текстовом редакторе.<br>Замените **Install** на **Remove** и сохраните файл.<br>**pkgmgr/n: \<update\> . XML**                                 |
|                    Настройка автоматических обновлений                    |          Чтобы проверить текущее значение, выполните следующие действия: **cscript%systemroot%\system32\scregedit.WSF/ау/v \* \* <br> для включения автоматического обновления: \* \* cscript скрежедит. WSF/ау 4** <br>Отключение автоматического обновления: **cscript%systemroot%\SYSTEM32\SCREGEDIT.WSF/ау 1**          |
...
| Преобразование тома в файловую систему NTFS |        **Преобразование \<volume letter\> /FS: NTFS**         |
...
|Задача|Get-Help|
|----|-------|
|Добавление драйвера нового устройства|Скопируйте драйвер в папку на сайте% хомедриве% \\ \<driver folder\> . Выполните команду **PnPUtil-i-a% хомедриве% \\ \<driver folder\> \\ \<driver\> . INF**|
|Удаление драйвера устройства|Чтобы получить список загруженных драйверов, выполните команду **SC Query Type = драйвер**. Затем выполните команду **SC \<service_name\> Delete** .|
