...
description: Описывает установку и управление агрегатором ведения журнала инвентаризации программного обеспечения — Software-Inventory-Logging
ms.topic: article
...
Средство инвентаризации программного обеспечения устанавливается на Windows Server, но не включено в комплект установки Windows Server. Чтобы установить это средство, сначала скачайте его бесплатно из центра загрузки Windows: [Средство инвентаризации программного обеспечения 1.0 для Windows Server](https://www.microsoft.com/download/details.aspx?id=49046).
...
4.  Откройте **WMIMgmt. msc** с параметром **начать** -> **Запуск**.
...
14. Нажмите кнопку **ОК** в окне **свойства** .
...
-   Вы **вошли на сервер с учетной записью, обладающей правами администратора** на сервере.
...
3.  Если вы принимаете условия лицензионного соглашения, установите флажок принятие соглашения и нажмите кнопку **Далее**.
...
1.  Откройте **PowerShell** с правами администратора и введите `Stop-SilAggregator` . Средство инвентаризации программного обеспечения будет остановлено, когда в командной строке появится результат.
...
|`Publish-SilReport`|Используется для создания отчетов из базы данных инвентаризации программного обеспечения. **Примечание.** Обработка куба на агрегаторе выполняется раз в день. Поэтому данные, полученные средством инвентаризации программного обеспечения, появятся в отчетах только на следующий день.|
...
-   В средстве инвентаризации программного обеспечения:
    ...
    -   Выполните команду `Start-SilAggregator`.
    ...
    -   Выполните команду `Add-SilVMHost`.

        Например, `add-silvmhost –vmhostname contoso1 –hostcredential get-credential`.
    ...
    -   Выполните команду `Set-SilAggregator –AddCertificateThumbprint` "отпечаток сертификата клиента".
...
-   На серверах Windows Server, которые будут участвовать в инвентаризации, запустите PowerShell с правами администратора и выполните следующие команды:
    ...
    -   Выполните команду `Set-SilLogging –TargetUri "https://contososilaggregator" –CertificateThumbprint "your client certificate's thumbprint"`.
    ...
    -   Выполните команду `Start-SilLogging`.
    ...
    -   Выполните команду `Publish-SilData`.
...
![Схема агрегатора для ведения журнала инвентаризации программного обеспечения](../media/software-inventory-logging/SILA_Architecture.png)
...
-   `Copy-Item ${firstAvailableDriveLetter}:\`**<файл CertificateName. pfx в каталоге нового диска> c:\<location of your choice>**
...
![Изображение отчета агрегатора для ведения журнала инвентаризации программного обеспечения](../media/software-inventory-logging/SILA_Report.png)
...
-   Игнорировать все значения, которые являются серыми или затемненными. Это артефакты создания отчета из куба SSAS.
...
|Имя компьютера|Сетевое имя или полное доменное имя для физического узла, который успешно опрашивается средством инвентаризации программного обеспечения.<p>Воспользуйтесь командлетом Get-SilVMHost, чтобы найти узлы, которые были добавлены, но не опрашиваются или больше не опрашиваются. Отображается последний успешный опрос.|
...
|Одновременно работающие виртуальные машины Windows Server по узлам|Счетчик одновременно работающих виртуальных машин Windows Server на узле. Наибольшее количество в течение месяца для этого узла является отметкой пиковой активности, полученной на этот момент времени.<p>См. раздел **Отметка пиковой активности** в этой документации.<p>Физические узлы с установленным сервером Windows Server или с установленным сервером и неизвестными работающими виртуальными машинами всегда имеют значение один в счетчике. Если по крайней мере одна известная виртуальная машина Windows Server работает на узле и Windows Server работает на самом узле, то операционная система узла не входит в счетчик.|
...
|Дата и время опросов|Дата и время последней отметки пиковой активности для одновременно работающих виртуальных машин Windows Server на этом физическом узле.<p>См. раздел **Дата и время опроса** этой документации.|
...
|Последние дата и время работы узла|Дата и время, когда средство инвентаризации программного обеспечения последний раз получило данные по HTTPS от физического узла Windows Server.<p>Поддерживается возможность того, чтобы физические узлы, на которых работают Windows Server и Hyper-V, могли включать инвентаризацию программного обеспечения и отправлять данные по HTTPS средству инвентаризации программного обеспечения.|

## <a name="sil-aggregator-cmdlets-detail"></a>Сведения о командлетах средства инвентаризации программного обеспечения
Ниже приводятся сведения о командлетах для средства инвентаризации программного обеспечения. Полную документацию по командлетам см. в разделе [Командлеты PowerShell для средства инвентаризации программного обеспечения](/previous-versions/windows/powershell-scripting/mt548455(v=wps.640)).
...
![Изображение завершенного командлета Get-Аггрегатордата](../media/software-inventory-logging/SILA_Get-SILAggregator.png)
...
-   При использовании Windows Management Framework в своей среде:
    ...
    -   Убедитесь, что на серверах с установленным сила не установлен WMF 5,1.  Можно получить сообщение об ошибке в журнале событий, относящемся к библиотеке DLL **"mpunits.dll"**.  Это предотвратит правильную работу.  Для сила требуется только WMF 4,0.
...
[Средство инвентаризации программного обеспечения 1.0 для Windows Server](https://www.microsoft.com/download/details.aspx?id=49046)<br>
[Командлеты PowerShell для средства инвентаризации программного обеспечения](/previous-versions/windows/powershell-scripting/mt548455(v=wps.640))<br>
[Командлеты PowerShell для инвентаризации программного обеспечения](/powershell/module/softwareinventorylogging/?view=winserver2012R2-ps)<br>
[Общие сведения об инвентаризации программного обеспечения](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dn268301(v=ws.11))<br>
[Управление инвентаризацией программного обеспечения](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dn383584(v=ws.11))