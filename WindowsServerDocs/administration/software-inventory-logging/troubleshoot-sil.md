...
![Схема платформы SIL](../media/software-inventory-logging/image1.png)
...
**Get-силаггрегатор-VmHostName &lt; полное доменное имя опроса физического узла, в котором имеется значение в столбце недавних опросов при использовании командлета &gt; Get-силвмхост**: это приведет к созданию информации из базы данных о физическом узле, даже перед обработкой куба.
...
1. Откройте **PowerShell** с правами администратора и выполните командлет **Get-силвмхост** .
    ...
    -   Если указаны узлы, **HostType** и **хипервисортипе** перечислены со значениями, которые не являются **неизвестными**, т. е. Windows, HyperV, Ubuntu и Xen и т. д., но в столбце " **недавние опросы** " нет **даты и времени** , опрос еще не выполнен.
        ...
        -   Если прошло час с момента добавления узла, проверьте, выполняется ли задача опроса: в **планировщик задач**выберите модуль **агрегации журнала инвентаризации программного обеспечения** в **Microsoft** &gt; **Windows** и проверьте журнал задачи.
