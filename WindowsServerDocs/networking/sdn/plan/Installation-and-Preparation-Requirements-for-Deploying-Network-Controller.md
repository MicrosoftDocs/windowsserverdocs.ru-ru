...
- Все виртуальные машины и компьютеры, запланированные как узлы сетевого контроллера, должны работать под управлением Windows Server 2019 или 2016 Datacenter Edition.
- На любом компьютере или виртуальной машине, на которой устанавливается сетевой контроллер, должен работать Windows Server 2019 или 2016 Datacenter Edition.
...
1. Разрешить динамическое обновление DNS для зоны.
   ...
   a. Откройте диспетчер DNS и в дереве консоли щелкните правой кнопкой мыши соответствующую зону и выберите пункт **Свойства**.

   b. На вкладке **Общие** убедитесь, что зона принадлежит к типу **первичная** или **Active Directory-Integrated**.

   c. В меню **динамические обновления** убедитесь, что выбран параметр **только защита** , а затем нажмите кнопку **ОК**.

2. Настройка разрешений безопасности зоны DNS для узлов сетевого контроллера

   a.  На вкладке **Безопасность** нажмите кнопку **Дополнительно**.

   b. В окне **Дополнительные параметры безопасности** нажмите кнопку **Добавить**.

   c. Щелкните **Выбор участника**.

   d. В диалоговом окне **Выбор пользователя, компьютера, учетной записи службы или группы** нажмите кнопку **типы объектов**.

   д) В списке **типы объектов** выберите **Компьютеры**, а затем нажмите кнопку **ОК**.

   е) В диалоговом окне **Выбор пользователя, компьютера, учетной записи службы или группы** введите NetBIOS-имя одного из узлов сетевого контроллера в развертывании, а затем нажмите кнопку **ОК**.
...
![Развертывание сетевого контроллера](../../media/Plan-a-Software-Defined-Network-Infrastructure/SDN-NC-Planning.png)
...
Для обеспечения высокой доступности существует два или более узла SLB/МУЛЬТИПЛЕКСОРа.

![Развертывание сетевого контроллера и программной подсистемы балансировки нагрузки](../../media/Plan-a-Software-Defined-Network-Infrastructure/SDN-SLB-Deployment.png)
...
![Сетевой контроллер, программное Load Balancer и развертывание шлюза RAS](../../media/Plan-a-Software-Defined-Network-Infrastructure/SDN-GW-Deployment.png)
