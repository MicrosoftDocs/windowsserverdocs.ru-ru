...
description: В этом разделе объясняется, как можно предоставить службы DNS для рабочих нагрузок размещенного клиента с помощью внутренней службы DNS (iDNS), интегрированной с программно-определяемой сетью в Windows Server 2019 и 2016.
manager: grcusanz
ms.topic: how-to
...
ms.locfileid: "98716330"
---
# <a name="internal-dns-service-idns-for-sdn"></a>Внутренняя служба DNS (iDNS) для SDN

>Применяется к: Windows Server 2019, Windows Server 2016

Если вы работаете с поставщиком облачных служб \( \) (CSP) или Enterprise, который планирует развертывать программно определенную сеть \( Sdn \) в Windows Server 2019 или 2016, вы можете предоставлять службы DNS для рабочих нагрузок размещенных клиентов с помощью внутренней службы DNS \( IDNS \) , интегрированной с Sdn.

Виртуальным машинам и приложениям размещенных виртуальных машин \( \) требуется, чтобы служба DNS могла обмениваться данными в собственных сетях и с внешними ресурсами в Интернете. С помощью iDNS можно предоставить клиентам службы разрешения имен DNS для изолированного, локального пространства имен и для Интернет-ресурсов.
...
- [Развертывание инфраструктуры программно-конфигурируемой сети с помощью скриптов](../deploy/deploy-a-software-defined-network-infrastructure-using-scripts.md)
...
>[!NOTE]
>Это выдержка из раздела **конфигурации конфигуреиднс** в SDNExpress.ps1. Дополнительные сведения см. в статье [Развертывание инфраструктуры программно-определяемой сети с помощью VMM](../deploy/deploy-a-software-defined-network-infrastructure-using-scripts.md).
...
>[!NOTE]
>Это выдержка из раздела **конфигурации конфигуреиднспрокси** в SDNExpress.ps1. Дополнительные сведения см. в статье [Развертывание инфраструктуры программно-определяемой сети с помощью VMM](../deploy/deploy-a-software-defined-network-infrastructure-using-scripts.md).
...
Дополнительные сведения см. в разделе [Enable-NetFirewallRule](/powershell/module/netsecurity/enable-netfirewallrule?view=winserver2012r2-ps).
...
Дополнительные сведения см. в статьях [Создание виртуальной машины и подключение к виртуальной сети клиента или VLAN](../manage/create-a-tenant-vm.md).
...
>[!NOTE]
>Эти сведения содержатся в разделе **Конфигурация аттачтовиртуалнетворк** в SDNExpressTenant.ps1. Дополнительные сведения см. в статье [Развертывание инфраструктуры программно-определяемой сети с помощью VMM](../deploy/deploy-a-software-defined-network-infrastructure-using-scripts.md).
