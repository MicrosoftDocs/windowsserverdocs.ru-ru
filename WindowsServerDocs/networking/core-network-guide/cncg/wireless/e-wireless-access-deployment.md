...
- **Шифрование**. Укажите \- предпочтительный вариант "WPA2 Enterprise" \( или " \) WPA \- Enterprise" и либо Предпочтительный алгоритм AES, либо \( \) шифр шифрования TKIP, в зависимости от того, какие версии поддерживаются сетевыми адаптерами беспроводного клиентского компьютера.
...
## <a name="configure-wireless-network-ieee-80211-policies"></a><a name="bkmk_policies"></a>Настройка беспроводных сетевых \( политик IEEE 802,11 \)
...
### <a name="activate-default-wireless-network-ieee-80211-policies"></a><a name="bkmk_activate"></a>Активировать политики беспроводных сетей \( IEEE 802,11 по умолчанию \)
...
2. На левой панели GPME дважды \- щелкните **Конфигурация компьютера**, дважды \- щелкните **политики**, дважды \- щелкните **Параметры Windows**, а затем дважды \- щелкните **Параметры безопасности**.
...
16. Нажмите **Настроить**. В диалоговом окне **Свойства EAP MSCHAPv2** убедитесь, что выбран параметр **автоматически использовать имя входа и пароль Windows и \( \) домен, если** он установлен, и нажмите кнопку **ОК**.
...
8. В **дополнительных параметрах**, если используются методы проверки подлинности, отличные от EAP и PEAP, и если NAS поддерживает использование атрибута проверки подлинности сообщения, то выберите **сообщения запроса доступа должен содержать \- атрибут Message Authenticator**.

9. Нажмите кнопку **OK**. NAS появится в списке клиентов RADIUS, настроенных на сервере политики сети.
...
Эту процедуру можно использовать для создания политик запросов на подключение и сетевых политик, необходимых для развертывания 802.1 X, \- поддерживающих беспроводные точки доступа, в качестве \- \( клиентов RADIUS службы пользователей \) на сервере RADIUS, на котором работает сервер политики сети \( \) .
...
8.  Нажмите кнопку **OK**. Диалоговое окно Изменение защищенных свойств EAP закроется, и откроется мастер **настройки 802.1 x** . Нажмите кнопку **Далее**.
...
3. Присоедините новый беспроводной компьютер к домену \( , например через подключение Ethernet, которое не требует проверки подлинности IEEE 802.1 x, \) и добавьте загрузочный профиль беспроводной сети на компьютер с помощью команды **Netsh WLAN Add Profile** .
    ...
    >[!NOTE]
    >Дополнительные сведения см. в разделе команды Netsh для беспроводной локальной сети \( WLAN \) по адресу [http: \/ \/ TechNet.Microsoft.com \/ Library \/ dd744890. aspx](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/dd744890(v=ws.10)).
...
3. Кроме того, предоставьте пользователям имя пользователя и пароль учетных данных локального компьютера и \( \) \( имя учетной записи пользователя домена доменных учетных данных \) в формате *имя_домена \\ имя пользователя*, а также процедуры для "присоединение компьютера к домену" и "вход в домен", как описано в разделе " [основное сетевое](../../core-network-guide.md)описание Windows Server 2016".
...
8. В окне **успешного добавления**_идентификатора SSID сети_ нажмите кнопку **изменить параметры подключения**.
