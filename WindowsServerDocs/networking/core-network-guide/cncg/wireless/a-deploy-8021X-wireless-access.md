---
title: Развертывание беспроводного доступа с проверкой подлинности на основе паролей 802.1 X
...
В этом руководство объясняется, как выполнить сборку на основе основной сети, предоставив инструкции о том, как развернуть институт инженеров по \( \) беспроводной связи IEEE 802.1 x \- с проверкой подлинности IEEE 802,11 с помощью защищенного протокола расширенной проверки подлинности — протокола проверки подлинности Microsoft Challenge, версия 2 \( PEAP \- MS \- CHAP v2 \) .
...
Существует два варианта регистрации серверов аутентификации с сертификатами сервера для использования с проверкой подлинности 802.1 X. развертывайте собственную инфраструктуру открытых ключей с помощью Active Directory служб сертификации \( AD CS \) или используйте сертификаты сервера, зарегистрированные ЦС общедоступного центра сертификации \( \) .
...
- В пошаговом руководством по основным сетям Windows Server 2016 выполняется [Развертывание сертификатов сервера для беспроводных и беспроводных развертываний 802.1 x](../server-certs/deploy-server-certificates-for-802.1x-wired-and-wireless-deployments.md?f=255&MSPPError=-2147217396) в формате HTML в технической библиотеке.
...
|802.11a|С \- частотой от ISM \( 5,725 до 5,875 ГГц\)|54 Мбит/с|Обычно не используется из-за расходов и ограниченного диапазона.|
|802.11g|\-Диспетчер управления с частотой|54 Мбит/с|Широко используемые. устройства 802.11 g совместимы с устройствами 802.11 b.|
...
>[!NOTE]
>При настройке политик беспроводной сети необходимо выбрать **WPA2 \- Enterprise**, **WPA \- Корпоративная** или **Open With 802.1 x** , чтобы получить доступ к параметрам EAP, необходимым для беспроводных развертываний с проверкой подлинности 802.1 x.
...
**РЭ \- Защищенный доступ Wi- \( Fi \- \) Enterprise** WPA — это промежуточный стандарт, разработанный WiFi Alliance для обеспечения соответствия протоколу безопасности беспроводной сети 802,11. Протокол WPA был разработан в ответ на ряд серьезных уязвимостей, обнаруженных в предыдущем WEP-протоколе Wired Privacy \( \) .
...
2. **AES \- ГКМП**. Протокол галоис режима счетчиков \( ГКМП \) поддерживается в 802.11 AC, более эффективен, чем AES \- CCMP, и обеспечивает лучшую производительность для беспроводных клиентов. ГКМП использует ключи шифрования AES 256 разрядов.
...
### <a name="active-directory-doman-services-ad-ds"></a>Active Directory домен Services \( AD DS\)
...
Протокол расширенной проверки подлинности \( EAP \) расширяет \- \- протокол \( PPP \) , предоставляя дополнительные методы проверки подлинности, использующие обмен учетными данными и сведениями о произвольной длине. При использовании проверки подлинности EAP для успешной проверки подлинности клиент доступа к сети и средства проверки подлинности, \( такие как NPS, \) должны поддерживать один и тот же тип EAP. Windows Server 2016 включает в себя инфраструктуру EAP, поддерживает два типа EAP и возможность передавать сообщения EAP в НПСС. С помощью протокола EAP можно поддерживать дополнительные схемы проверки подлинности, называемые *типами EAP*. Типы EAP, поддерживаемые Windows Server 2016:
...
Протокол PEAP \- MS \- CHAP v2 проще развертывать по сравнению \- с EAP TLS, так как проверка подлинности пользователя выполняется с помощью пароля \- и имени пользователя учетных данных \( \) , а не сертификатов или смарт-карт. Сертификат должен иметь только сервер политики сети или другие серверы RADIUS. В процессе проверки подлинности NPS-сертификат используется сервером политики сети для подтверждения его личности для PEAP-клиентов.
...
2.  Сервер политики сети проверяет подлинность пользователя. После успешной проверки подлинности сервера политики сети клиент отправляет \- учетные данные пользователя на основе пароля на сервер политики сети, который проверяет учетные данные пользователя на соответствие с базой данных учетных записей пользователей в Active Directory домен Services \( AD DS \) .
