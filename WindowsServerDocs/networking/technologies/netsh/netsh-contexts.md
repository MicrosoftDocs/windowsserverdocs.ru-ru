...
Необязательный параметр. Указывает домен, в котором находится учетная запись пользователя. По умолчанию используется локальный домен, если не указан элемент _DomainName\\*.
...
Необязательный параметр. Указывает пароль для учетной записи пользователя, заданный с помощью _ *-u** *UserName*.

***`NetshCommand`** _

Необязательный элемент. Указывает команду _ *netsh**, которую требуется выполнить.
