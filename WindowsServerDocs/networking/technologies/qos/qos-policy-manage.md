...
author: JasonGerend
ms.date: 08/07/2020
ms.openlocfilehash: 8b9b959bc3911311e3ef3622fda4a4caca0bcb58
ms.sourcegitcommit: db4c35ebe56d561768d2a657da9e6d6a791457bd
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/04/2021
ms.locfileid: "101829233"
---
...
URL-адрес должен соответствовать  [RFC 1738](https://tools.ietf.org/html/rfc1738)в виде `http[s]://<hostname\>:<port\>/<url-path>` . Можно использовать подстановочный знак, `‘*'` , для `<hostname>` и/или `<port>` , например `https://training.\*/, https://\*.\*` , но подстановочный знак не может заменять подстроку `<hostname>` или `<port>` .

Иными словами, `https://my\*site/` и `https://\*training\*/` не являются допустимыми.

При необходимости можно установить флажок **включить подкаталоги и файлы** , чтобы выполнить сопоставление для всех подкаталогов и файлов, следующих за URL-адресом. Например, если этот параметр установлен и URL-адрес имеет значение `https://training` , то политика QoS будет рассматривать запросы на предмет ` https://training/video` соответствия.
...
[Wi-Fi Alliance](https://go.microsoft.com/fwlink/?LinkId=160769) установила сертификацию для беспроводного мультимедиа \( WMM \) , определяющего четыре категории доступа \( WMM_AC \) для определения приоритетов сетевого трафика, передаваемого в \- беспроводной сети Wi Fi. Категории доступа включают \( в себя самый высокий и низкий приоритет \) : голоса, видео, лучшие усилия и фоновый режим; соответственно сокращаются как во, VI, BK. Спецификация WMM определяет, какие значения DSCP соответствуют каждой из четырех категорий доступа:
...
Например, policy_A указывает только имя приложения (app.exe) и policy_B Указывает конечный IP-адрес 192.168.1.0/24. Когда эти политики QoS конфликтуют \(app.exe отправляет трафик на IP-адрес в диапазоне 192.168.4.0/24 \) , применяется policy_A.
