...
ms.locfileid: "87952120"
---
# <a name="manage-nps-templates"></a>Управление шаблонами сервера политики сети

>Применяется к: Windows Server (Semi-Annual Channel), Windows Server 2016

Шаблоны NPS сервера политики сети можно \( использовать \) для создания элементов конфигурации, таких как клиенты протокол RADIUS \( RADIUS \) или общие секреты, которые можно повторно использовать на локальном сервере политики сети и экспортировать для использования в других НПСС.
...
- **IP-фильтры**. Этот шаблон позволяет создавать фильтры IPv6 для протокола IP версии 4 (IPv4) и протокола IP версии 6 \( \) , которые можно повторно использовать, \( выбрав шаблон в соответствующем расположении в консоли NPS \) при настройке сетевых политик.
...
Например, если настроить клиент RADIUS в консоли NPS в разделе **Клиенты и серверы RADIUS**, необходимо изменить конфигурацию NPS и выполнить один шаг в настройке NPS для взаимодействия с одним из серверов доступа к сети. \(Следующим шагом является настройка \( NAS \) для подключения к серверу политики сети.\)
