...
Используйте эти рекомендации для упрощения анализа сети.

- Обратитесь к справке командной строки для получения аргументов и возможностей (например, пктмон Start Help).
- Настройте фильтры пакетов, соответствующие вашему сценарию (Добавление фильтра пктмон).
- Проверьте счетчики пакетов во время эксперимента для представления высокого уровня (счетчики пктмон).
- Просмотрите журнал для получения подробного анализа (пктмон Format пктмон. ETL).
...
| Функция                                                                  | ВЕРСИЯ 1809 (B:17763) | ВЕРСИЯ 1903 (B:18362) | ВЕРСИЯ 2004 (B:19041) |
|:---------------------------------------------------------------------------:|:----------------:|:----------------:|:----------------:|
...
- Сейчас поддерживается только трафик Ethernet. Поддержка беспроводного трафика будет добавлена в последующих выпусках.
- Пакеты, перепадает из брандмауэра Windows, пока не видны через монитор пакетов. 
...
### <a name="pktmon-command-syntax-and-formatting"></a>[Синтаксис команд и форматирование Pktmon](pktmon-syntax.md)
...
### <a name="packet-monitoring-extension-in-windows-admin-center"></a>[Расширение мониторинга пакетов в Windows Admin Center](pktmon-wac-extension.md)
...
### <a name="wireshark-pcapng-format-support"></a>[Поддержка Wireshark (формат PCAPNG)](pktmon-pcapng-support.md)
