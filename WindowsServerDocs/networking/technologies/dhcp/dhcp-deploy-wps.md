...
1. Одна виртуальная машина под Windows Server 2016 настроена в качестве контроллера домена с домен Active Directoryными службами, подключенными к созданному внутреннему виртуальному коммутатору. Для соответствия этому руководству этот сервер должен иметь статически настроенный IP-адрес 10.0.0.2. Сведения о развертывании AD DS см. в разделе **развертывание DC1** в [сетевом каталоге](../../core-network-guide/core-network-guide.md#BKMK_deployADDNS01)Windows Server 2016 Core.
...
2. Один физический компьютер под Windows Server 2016, настроенный как контроллер домена с домен Active Directoryными службами. Для соответствия этому руководству этот сервер должен иметь статически настроенный IP-адрес 10.0.0.2. Сведения о развертывании AD DS см. в разделе **развертывание DC1** в [сетевом каталоге](../../core-network-guide/core-network-guide.md#BKMK_deployADDNS01)Windows Server 2016 Core.
...
Дополнительные сведения см. в разделе Создание виртуального **коммутатора с помощью диспетчера Hyper-V** статьи [Создание виртуальной сети](/virtualization/hyper-v-on-windows/quick-start/connect-to-network).
...
- [New-Нетипаддресс](/powershell/module/nettcpip/New-NetIPAddress)
- [Set-Днсклиентсервераддресс](/powershell/module/dnsclient/Set-DnsClientServerAddress)
...
### <a name="join-the-computer-to-the-domain-optional"></a>Присоединение компьютера к домену \( необязательно\)
...
- [Add-Дхкпсервериндк](/powershell/module/dhcpserver/add-dhcpserverindc)
- [Get-Дхкпсервериндк](/powershell/module/dhcpserver/get-dhcpserverindc)

### <a name="notify-server-manager-that-post-install-dhcp-configuration-is-complete-optional"></a>Уведомлять диспетчер сервера о том, что учетная запись \- конфигурации DHCP завершена \( необязательно\)
...
### <a name="set-server-level-dns-dynamic-update-configuration-settings-optional"></a>Задать параметры конфигурации динамического обновления DNS на уровне сервера \( необязательно\)
...
3. Убедитесь, что DHCP-сервер разрешен в Active Directory, выполнив следующую команду, чтобы получить список разрешенных DHCP-серверов из Active Directory. [Get-дхкпсервериндк](/powershell/module/dhcpserver/get-dhcpserverindc).
...
- [Модуль DhcpServer](/powershell/module/dhcpserver/)
