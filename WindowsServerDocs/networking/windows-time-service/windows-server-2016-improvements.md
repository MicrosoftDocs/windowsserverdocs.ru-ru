...
Кроме того, уровень страты, сведения о котором узел передает гостевой системе, более прозрачен. Ранее узел передавал сведения о фиксированном значении страты 2, независимо от точности. Благодаря изменениям в Windows Server 2016 узел передает сведения о страте, превышающей страту узла. Это обеспечивает лучшие показатели времени для виртуальных гостевых систем. W32time определяет cтрату узла стандартными средствами на основе исходного времени. Присоединенные к домену гостевые системы Windows 2016 будут находить наиболее точные часы, а не стандартные часы узла. По этой причине мы рекомендуем вручную отключить параметр поставщика времени Hyper-V для компьютеров, находящихся в домене в Windows 2012 R2 и более ранних версий.
...
| |*Периодичность обновления часов*|Н/Д|Один раз в день|Раз в час|
|**Контроллер домена**||||
| |*Сервер времени*|PDC/GTIMESERV|Н/Д|PDC/GTIMESERV|
| |*Частота опросов*|64 — 1024 секунд|Н/Д|1024 — 32768 секунд|
| |*Периодичность обновления часов*|Один раз в секунду|Н/Д|Раз в час|
...
| |Сервер времени|Выбор наилучшего варианта на основе страты сервера узла и времени|Выбор наилучшего варианта на основе страты сервера узла и времени|По умолчанию используется узел|
...
![Схема топологии службы времени Windows, в которой в первом узле Hyper-V выполняется только один клиент дочернего домена.](../media/Windows-Time-Service/Windows-2016-Accurate-Time/topology1.png)

![Схема топологии службы времени Windows с двумя клиентами дочерних доменов: один выполняется в первом узле Hyper-V, а другой — во втором.](../media/Windows-Time-Service/Windows-2016-Accurate-Time/topology2.png)
...
![Схема топологии службы времени Windows с выделенными сервером PDC корневого домена и клиентскими серверами дочернего домена в первом узле Hyper-V.](../media/Windows-Time-Service/Windows-2016-Accurate-Time/topology3.png)
...
![Схема, на которой представлен корневой домен.](../media/Windows-Time-Service/Windows-2016-Accurate-Time/chart1.png)
...
![Схема, на которой представлен клиент дочернего домена.](../media/Windows-Time-Service/Windows-2016-Accurate-Time/chart2.png)
...
![Схема, на которой представлена проверка на дальних расстояниях.](../media/Windows-Time-Service/Windows-2016-Accurate-Time/chart3.png)
...
Некоторые поставщики сети используют обновления драйверов, повышающие производительность с точки зрения задержки драйвера и буферизации пакетов UDP. Обратитесь к поставщику сети, чтобы узнать, есть ли обновления для получения помощи по пропускной способности UDP.
