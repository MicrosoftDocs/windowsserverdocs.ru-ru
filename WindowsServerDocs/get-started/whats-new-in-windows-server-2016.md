...
description: Новые возможности для вычислений, удостоверений, управления, автоматизации, сетей, безопасности и хранения в Windows Server 2016.
ms.date: 05/21/2019
ms.topic: article
...
Дополнительные сведения и инструкции по работе с экранированными виртуальными машинами см. в [руководстве по проверке защищенных структур для Windows Server 2016 (TPM) и экранированных виртуальных машин](../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms-top-node.md).

## <a name="identity-and-access"></a>[Удостоверения и доступ](../identity/Identity-and-Access.yml)
...
##  <a name="administration"></a>[Администрирование](../administration/manage-windows-server.yml)
...
Дополнительные сведения о тестировании возможностей DSC в Windows Management Framework 5 см. в серии записей блога [Validate features of PowerShell DSC](https://devblogs.microsoft.com/powershell/validate-features-of-powershell-dsc/) (Проверка компонентов PowerShell DSC). Чтобы скачать Windows Management Framework 5.1, перейдите в [этот раздел](/powershell/scripting/wmf/setup/install-configure).
...
Дополнительные сведения см. в статье, посвященной [PowerShell на стороне синих](https://devblogs.microsoft.com/powershell/powershell-the-blue-team/).

## <a name="networking"></a>[Сетевое взаимодействие](../networking/index.yml)
...
Защита потока управления (CFG) — это компонент безопасности платформы, предназначенный для борьбы с уязвимостями на базе повреждения памяти. Дополнительные сведения см. в статье [Защита потока управления](/windows/win32/secbp/control-flow-guard).


## <a name="storage"></a>[Хранилище](../storage/storage.yml)
...
## <a name="failover-clustering"></a>[Отказоустойчивая кластеризация](../failover-clustering/whats-new-in-failover-clustering.md)
...
- Диспетчер ресурсов может использовать новый интерфейс для метода Rejoin, чтобы определить результат сомнительной транзакции после перезагрузки базы данных из-за ошибки. Дополнительные сведения см. в статье о [IResourceManagerRejoinable::Rejoin](/previous-versions/windows/desktop/mt203799(v=vs.85)).

- Максимальный размер DSN-имени увеличен с 256 до 3072 байтов. Дополнительные сведения см. в статьях об [IDtcToXaHelperFactory::Create](/previous-versions/windows/desktop/ms686861(v=vs.85)), [IDtcToXaHelperSinglePipe::XARMCreate](/previous-versions/windows/desktop/ms679248(v=vs.85)) и [IDtcToXaMapper::RequestNewResourceManager](/previous-versions/windows/desktop/ms680310(v=vs.85)).
