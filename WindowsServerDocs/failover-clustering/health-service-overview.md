...
Если `Disks` раздел указан, то `Disk` для объединения пулов разрешены только перечисленные диски (AS). Все непоставленные диски не могут присоединяться к пулам, что эффективно исключает их использование в рабочей среде. Если этот раздел оставлен пустым, то всем дискам будет разрешено присоединяться к пулам.

Если `Cache` раздел указан, `CacheDisk` для кэширования используются только перечисленные диски (как). Если этот раздел оставлен пустым, Локальные дисковые пространства пытается [угадать на основе типа носителя и типа шины](../storage/storage-spaces/understand-the-cache.md#cache-drives-are-selected-automatically). Перечисленные здесь диски также должны быть перечислены в разделе `Disks` .
...
Чтобы получить список нескольких дисков, просто добавьте `<Disk>` Дополнительные `<CacheDisk>` теги или.
