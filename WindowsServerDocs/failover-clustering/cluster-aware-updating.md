...
description: Cluster-Aware обновление (CAU) автоматизирует установку обновления программного обеспечения в кластерах под Windows Server.
ms.topic: article
manager: lizross
author: JasonGerend
ms.author: jgerend
...
> [!NOTE]
> При обновлении кластеров [Локальные дисковые пространства](../storage/storage-spaces/storage-spaces-direct-overview.md) рекомендуется использовать Cluster-Aware обновление.

## <a name="feature-description"></a><a name="BKMK_OVER"></a>Описание функции
Cluster-Aware обновление — это автоматизированная функция, которая позволяет обновлять серверы в [отказоустойчивом кластере](failover-clustering-overview.md) с незначительной потерей доступности в процессе обновления. Во время прогона обновления Cluster-Awareное обновление прозрачно выполняет следующие задачи:
...
Ниже приведено описание важных функциональных возможностей обновления Cluster-Aware.
...
Cluster-Aware обновление может координировать полную операцию обновления кластера в двух режимах:
...
### <a name="installing-cluster-aware-updating"></a>Установка обновления Cluster-Aware
...
- [Install-WindowsFeature](/powershell/module/servermanager/Install-WindowsFeature?view=winserver2012r2-ps) Командлет Windows PowerShell
...
> [!NOTE]
> -   Средства отказоустойчивой кластеризации в Windows Server 2012 нельзя использовать для управления обновлением Cluster-Aware в более новой версии Windows Server.
...
Чтобы включить режим самообновления, необходимо добавить Cluster-Aware обновление кластерной роли в отказоустойчивый кластер. Для этого используйте один из следующих методов.
- В Диспетчер сервера выберите **инструменты**  >  **обновление с поддержкой кластера**, а затем в окне Cluster-Aware обновление выберите **настроить параметры самостоятельного обновления кластера**.
- В сеансе PowerShell выполните командлет [Add-кауклустерроле](/powershell/module/clusterawareupdating/Add-CauClusterRole) .

Чтобы удалить CAU, удалите компонент отказоустойчивой кластеризации или средства отказоустойчивой кластеризации с помощью диспетчер сервера, командлета [uninstall-WindowsFeature](/powershell/module/servermanager/Uninstall-WindowsFeature) или \- средств командной строки DISM.
...
### <a name="starting-cluster-aware-updating"></a>Запуск Cluster-Aware обновления

##### <a name="to-start-cluster-aware-updating-from-server-manager"></a>Запуск Cluster-Aware обновления из диспетчер сервера
...
Дополнительные сведения об использовании Cluster-Aware обновления см. по следующим ссылкам.
...
-   [\-Командлеты обновления с поддержкой кластера в Windows PowerShell](/powershell/module/clusterawareupdating/)
