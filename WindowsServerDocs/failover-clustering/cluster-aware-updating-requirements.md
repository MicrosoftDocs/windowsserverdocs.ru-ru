...
ms.author: jgerend
ms.date: 08/06/2018
description: Требования к использованию Cluster-Aware обновления для установки обновлений в кластерах под Windows Server.
ms.openlocfilehash: 33a0b60d1ce7fa3f1faa20d8239b175ead2836b2
ms.sourcegitcommit: d08965d64f4a40ac20bc81b14f2d2ea89c48c5c8
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 12/08/2020
ms.locfileid: "96865773"
---
...
|Средства отказоустойчивости кластеров|Требуется во всех узлах кластера|— Требуется на удаленно \- обновляемом компьютере<br />— Требуется на всех узлах кластера для выполнения командлета [Save-каудебугтраце](/powershell/module/clusterawareupdating/Save-CauDebugTrace)|
...
-   Чтобы просмотреть или применить действия обновления с помощью пользовательского интерфейса CAU \( \) или командлетов обновления Cluster-Aware, необходимо использовать учетную запись домена с правами локального администратора и разрешениями на всех узлах кластера. Если учетная запись не имеет достаточных привилегий на каждом узле, при выполнении этих действий появится запрос в окне Cluster-Aware обновления, чтобы указать необходимые учетные данные. Чтобы использовать командлеты [обновления с поддержкой кластера](/powershell/module/clusterawareupdating/) , можно указать необходимые учетные данные в качестве параметра командлета.
...
-   Чтобы запустить CAU анализатор соответствия рекомендациям, необходимо использовать учетную запись с правами администратора на узлах кластера и права локального администратора на компьютере, который используется для запуска командлета [Test-каусетуп](/powershell/module/clusterawareupdating/Test-CauSetup) , или для анализа готовности к обновлению кластера с помощью окна Cluster-Aware обновление. Подробнее см. в подразделе [Проверка готовности к обновлению кластера](#BKMK_BPA).
...
Для использования Cluster-Aware обновления все узлы кластера должны быть настроены для удаленного управления. По умолчанию единственной задачей, которую необходимо выполнить для настройки узлов для удаленного управления, является [Включение правила брандмауэра, разрешающего автоматический перезапуск](#BKMK_FW).
...
|---------------|---|-----------------------|-------------------------|
|[Включение правила брандмауэра для обеспечения автоматического перезапуска](#BKMK_FW)|Отключен|Требуется во всех узлах кластера, если используется брандмауэр|Требуется во всех узлах кластера, если используется брандмауэр|
...
|[Включение Windows PowerShell 3.0 или 4.0 и удаленного взаимодействия Windows PowerShell](#BKMK_PS)|Активировано|Требуется во всех узлах кластера|Требуется во всех узлах кластера для выполнения следующих компонентов:<p>— Командлет [Save-каудебугтраце](/powershell/module/clusterawareupdating/Save-CauDebugTrace) .<br />— Предварительное \- Обновление PowerShell и \- выполнение скриптов после обновления во время прогона обновления<br />— Тестирование готовности к обновлению кластера с помощью окна Cluster-Aware обновление или командлета [Test \- Каусетуп](/powershell/module/clusterawareupdating/Test-CauSetup) Windows PowerShell|
...
Если в узлах кластера используется брандмауэр Windows, сделать это можно путем включения группы правил брандмауэра Windows **Удаленное завершение работы** в каждом узле кластера. При использовании окна Обновление Cluster-Aware для применения обновлений и настройки \- параметров автоматического обновления группа правил брандмауэра Windows для **удаленного завершения работы** автоматически включается на каждом узле кластера.
...
Группу правил брандмауэра для **удаленного завершения работы** также можно включить, указав параметр **– енаблефиреваллрулес** при выполнении следующих командлетов Cau: [Add-кауклустерроле](/powershell/module/clusterawareupdating/Add-CauClusterRole), [Invoke-каурун](/powershell/module/clusterawareupdating/Invoke-CauRun)и [сеткауклустерроле](/powershell/module/clusterawareupdating/Set-CauClusterRole).
...
2.  Выполните командлет [Set-WSManQuickConfig](/powershell/module/Microsoft.WsMan.Management/Set-WSManQuickConfig?view=powershell-6) или следующую команду из командной строки с повышенными привилегиями.
...
-   Выполните командлет [Enable-PSRemoting](/powershell/module/Microsoft.PowerShell.Core/Enable-PSRemoting) .

-   Настройте \- параметр Групповая политика уровня домена для служба удаленного управления Windows \( WinRM \) .

Дополнительные сведения о включении удаленного взаимодействия PowerShell см. в статье [об удаленных требованиях](/powershell/module/microsoft.powershell.core/about/about_remote_requirements?view=powershell-6).
...
## <a name="best-practices-recommendations-for-using-cluster-aware-updating"></a><a name="BKMK_BEST_PRAC"></a>Рекомендации по использованию Cluster-Aware обновления
...
2.  Выполните командлет [Test-CauSetup](/powershell/module/clusterawareupdating/Test-CauSetup) . Командлет можно запустить на локальном или удаленном компьютере, на котором установлен модуль отказоустойчивого кластера для Windows PowerShell (часть средств отказоустойчивой кластеризации). Его также можно выполнить в узле отказоустойчивого кластера.

> [!NOTE]
> -   Необходимо использовать учетную запись с правами администратора на узлах кластера и права локального администратора на компьютере, который используется для запуска командлета **Test \- каусетуп** или для анализа готовности к обновлению кластера с помощью окна Cluster-Aware обновление. Чтобы запустить тесты с помощью окна "обновление Cluster-Aware", необходимо войти в систему на компьютере с необходимыми учетными данными.
