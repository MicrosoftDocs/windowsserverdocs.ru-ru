...
author: JasonGerend
ms.date: 08/07/2020
ms.openlocfilehash: 3da52f980988eb1d90318ea1dc501c13dd1382bb
ms.sourcegitcommit: db4c35ebe56d561768d2a657da9e6d6a791457bd
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/04/2021
ms.locfileid: "101826899"
---
...
|---------|-----------------|
|Роль удаленного доступа|Эта роль устанавливается и удаляется с помощью консоли Диспетчера серверов. Он включает в себя DirectAccess, который ранее был компонентом в Windows Server 2008 R2, а также службы маршрутизации и удаленного доступа \( RRAS \) , который ранее был службой роли в \( роли сервера NPAS служб политики сети и доступа \) . Роль удаленного доступа включает два компонента.<p>-Always On VPN и службы маршрутизации и удаленного доступа \( RRAS \) VPN-DirectAccess и VPN управляются вместе в консоли управления удаленным доступом.<br />— Маршрутизация RRAS. Управление функциями маршрутизации RRAS осуществляется с помощью устаревшей консоли маршрутизации и удаленного доступа.<p>Существуют следующие зависимости.<p>-Службы IIS \( \) веб-сервер IIS. Эта функция необходима для настройки сервера сетевых расположений и веб-проверки по умолчанию.<br />— Внутренняя Database-Used Windows для локального учета на сервере удаленного доступа.|
...
|Network Load Balancing|Данный компонент обеспечивает балансировку нагрузки в кластере при помощи компонента балансировки сетевой нагрузки Windows.|
...
-   Для сценария External Load Balancer требуется специальное оборудование, \( т. е. F5 BigIP \) .
...
-   Помимо требований к программному обеспечению для одного сервера, существует ряд \- специфических требований к кластеру.

    -   На каждом сервере кластера \- имя субъекта сертификата IP-HTTPS должно совпадать с адресом ssASnoversion. Развертывание кластера поддерживает сочетание подстановочных знаков и сертификатов без \- подстановочных знаков на серверах кластера.
    ...
    -   Сертификаты IP- \- HTTPS и сервера сетевого расположения должны выдаваться с помощью того же метода, с которым был выдан сертификат для одного сервера. Например, если один сервер использует общедоступный ЦС центра сертификации \( , \) то все серверы в кластере должны иметь сертификат, выданный общедоступным центром сертификации. Или если один сервер использует самозаверяющий \- сертификат для IP-адреса \- HTTPS, все серверы в кластере должны быть аналогичными.
