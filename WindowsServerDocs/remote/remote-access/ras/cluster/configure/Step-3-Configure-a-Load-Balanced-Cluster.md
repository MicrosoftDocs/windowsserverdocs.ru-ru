---
title: Шаг 3. Настройка кластера Load-Balanced
description: Узнайте, как настроить балансировку нагрузки на одном сервере, настроить необходимые сертификаты и развернуть кластер.
manager: brianlic
ms.topic: article
...
ms.locfileid: "97947790"
---
# <a name="step-3-configure-a-load-balanced-cluster"></a>Шаг 3. Настройка кластера Load-Balanced
...
![Windows PowerShell Windows PowerShell — ](../../../../media/Step-3-Configure-a-Load-Balanced-Cluster/PowerShellLogoSmall.gif) * *_<em>эквивалентные команды</em>_* _
...
Членство в локальной группе _ *Administrators** (или эквивалентной) является минимальным требованием для выполнения этой процедуры.
...
1.  На сервере удаленного доступа нажмите кнопку _ * Пуск * *, а затем выберите **Управление удаленным доступом**. Если появится диалоговое окно **контроля учетных записей**, подтвердите, что отображаемое в нем действие именно то, которое требуется, и нажмите кнопку **Да**.
...
1.  На настроенном сервере DirectAccess щелкните _ * Пуск * *, а затем выберите **Управление удаленным доступом**. Если появится диалоговое окно **контроля учетных записей**, подтвердите, что отображаемое в нем действие именно то, которое требуется, и нажмите кнопку **Да**.
...
Нажатие кнопки _ *Удалить параметры конфигурации** приведет к удалению удаленного доступа и балансировки сетевой нагрузки (если настроено) со всех серверов в развертывании.
