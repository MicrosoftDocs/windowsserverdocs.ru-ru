...
description: Узнайте, как настроить сервер удаленного доступа для поддержки OTP.
manager: brianlic
ms.topic: article
...
После того как сервер RADIUS настроен с маркерами распространения программного обеспечения, порты связи открыты, был создан общий секрет, учетные записи пользователей, соответствующие Active Directory были созданы на сервере RADIUS, а сервер удаленного доступа был настроен как агент проверки подлинности RADIUS, сервер удаленного доступа должен быть настроен для поддержки OTP.
...
8.  В разделе **серверы ЦС OTP** выберите серверы ЦС, которые будут использоваться для регистрации сертификатов проверки подлинности клиента OTP, и нажмите кнопку **Добавить**. Щелкните **Далее**.
...
9. В разделе **шаблоны сертификатов OTP** нажмите кнопку **Обзор** , чтобы выбрать шаблон сертификата, используемый для регистрации сертификатов, выданных для проверки подлинности OTP.
    ...
    Нажмите кнопку **Обзор** , чтобы выбрать шаблон сертификата, который используется для регистрации сертификата, используемого сервером удаленного доступа, для подписания запросов на регистрацию сертификатов OTP. Нажмите кнопку **ОК**. Щелкните **Далее**.
...
![](../../../../media/Step-3-Configure-the-Remote-Access-Server-for-OTP/PowerShellLogoSmall.gif)**Эквивалентные команды** в Windows PowerShell Windows PowerShell
...
Так как клиенты DirectAccess используют смарт-карты для доступа к интрасети, можно использовать механизм проверки подлинности, компонент Windows Server 2008 R2 для управления доступом к ресурсам, таким как файлы, папки и принтеры, в зависимости от того, вошел ли пользователь в систему с помощью сертификата на основе смарт-карты. Для обеспечения надежности механизма проверки подлинности необходим функциональный уровень домена Windows Server 2008 R2.
