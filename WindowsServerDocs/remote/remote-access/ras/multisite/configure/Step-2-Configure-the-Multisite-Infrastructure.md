...
description: Дополнительные сведения о настройке многосайтового развертывания для изменения параметров сетевой инфраструктуры необходимо выполнить ряд действий.
manager: brianlic
ms.topic: article
...
1.  Чтобы заменить недоступный контроллер домена "DC2" на контроллер домена "DC3", выполните следующую команду:
    ...
    ![Схема, показывающая результирующую конфигурацию.](../../../../media/Step-2-Configure-the-Multisite-Infrastructure/DCAssoc2.png)
...
2.  Чтобы заменить недоступный контроллер домена DC1 на контроллере домена "DC3", выполните следующую команду:
    ...
    ![Схема, демонстрирующая сопоставление обновления с контроллером домена.](../../../../media/Step-2-Configure-the-Multisite-Infrastructure/DCAssoc3.png)
...
3.  Чтобы синхронизировать сопоставление контроллера домена с объектом групповой политики "точка входа 2" в объекте групповой политики "точка входа 1", выполните команду, чтобы заменить "DC2" на "DC3", и укажите сервер удаленного доступа, для которого объект групповой политики сервера не синхронизирован, в данном случае "DA1" для параметра *ComputerName* .
    ...
    ![Схема, показывающая окончательную конфигурацию.](../../../../media/Step-2-Configure-the-Multisite-Infrastructure/DCAssocFinal.png)
