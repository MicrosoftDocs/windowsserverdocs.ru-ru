...
## <a name="bgp-supported-deployment-topologies"></a><a name="bkmk_top"></a>Топологии развертывания, поддерживаемые протоколом BGP
...
### <a name="ras-vpn-site-to-site-gateway-with-bgp-at-enterprise-site-edge"></a><a name="bkmk_top1"></a>Шлюз VPN типа "сеть — сеть" для RAS с BGP на корпоративном сайте ребра
...
### <a name="third-party-gateway-with-bgp-at-enterprise-site-edge"></a><a name="bkmk_top2"></a>Сторонний шлюз с BGP на границе сайта предприятия
...
Если маршрутизатор BGP на корпоративном сайте 1 не может подключиться к маршрутизатору BGP корпоративного сайта 2 из-за сбоя подключения, маршрутизатор BGP сайта 1 динамически начинает изучать маршруты к сети корпоративного сайта 2 от маршрутизатора BGP поставщика служб шифрования, а трафик легко перенаправляется с сайта 1 на сайт 2 через маршрутизатор BGP Windows Server на CSP.
...
## <a name="bgp-features"></a><a name="bkmk_features"></a>Возможности BGP
