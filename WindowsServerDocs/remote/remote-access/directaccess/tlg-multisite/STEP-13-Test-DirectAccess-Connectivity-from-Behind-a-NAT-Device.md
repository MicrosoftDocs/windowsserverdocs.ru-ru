...
3. Проверьте вывод команды ipconfig. Этот компьютер теперь подключен к Интернету из-за устройства NAT и получил частный адрес IPv4. Туннелирование Teredo отключено, а клиент DirectAccess возвращается к использованию IP-HTTPS. При просмотре выходных данных команды ipconfig вы увидите раздел для адаптера туннеля ифттпсинтерфаце с IP-адресом, который начинается с 2001: db8:1: 1000 или 2001: db8:2: 2000 — это адрес IP-HTTPS, основанный на префиксах, которые были настроены при настройке DirectAccess. Шлюз по умолчанию, указанный для адаптера туннелирования Ифттпсинтерфаце, не отображается.
