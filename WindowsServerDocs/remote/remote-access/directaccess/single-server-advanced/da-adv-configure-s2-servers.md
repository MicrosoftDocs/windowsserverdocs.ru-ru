...
description: Узнайте, как настроить параметры клиента и сервера, необходимые для расширенного развертывания удаленного доступа, использующего один сервер удаленного доступа в смешанной среде IPv4 и IPv6.
manager: brianlic
ms.topic: article
...
![Ход выполнения установки. ](../../../media/Step-2-Configuring-DirectAccess-Servers/PowerShellLogoSmall.gif) * *_<em>эквивалентные команды Windows PowerShell</em>_* _
...
1.  На сервере удаленного доступа откройте консоль управления удаленным доступом: на экране _ *Start** введите **RAMgmtUI.exe** и нажмите клавишу ВВОД. Если появится диалоговое окно **контроля учетных записей**, подтвердите, что отображаемое в нем действие именно то, которое требуется, и нажмите кнопку **Да**.
...
-   [Шаг 3. Проверка развертывания](./da-adv-configure-s3-verify.md)