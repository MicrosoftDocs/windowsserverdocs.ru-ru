---
description: 'Дополнительные сведения о: приложение б. Настройка тестовой среды'
...
6. На странице **Выбор ролей сервера** выберите **Доменные службы Active Directory**. В диалоговом окне **Мастер добавления ролей и компонентов** щелкните **Добавить компоненты**, а затем нажмите кнопку **Далее**.
...
12. На странице **параметры контроллера домена** выберите режим работы домена и леса, как Windows Server 2012, укажите пароль DSRM <strong>pass@word1</strong> и нажмите кнопку **Далее**.
...
### <a name="build-the-file-server-and-ad-rms-server-file1"></a><a name="BKMK_FS1"></a>Создание файлового сервера и сервера AD RMS (FILE1)
...
3. Присоедините виртуальную машину к домену contoso.com, а затем войдите в FILE1 как CONTOSO\Administrator с помощью пароля <strong>pass@word1</strong> .
...
2.  Создайте следующие файлы с указанными атрибутами:
    ...
    -   Создайте на рабочем столе папку Regular Expressions. Создайте текстовый документ в папке **RegEx-SSN**. Введите в файле следующее содержимое, а затем сохраните и закройте файл: ^ (?! 000) ([0-7] \d {2} | 7 ([0-7] \d | 7 [012])) ([-]?) (?! 00) \d\d\3 (?! 0000) \d {4} $
...
#### <a name="install-active-directory-rights-management-services"></a><a name="BKMK_CS1"></a>Установка служб управления правами Active Directory
...
19. На экране **учетная запись службы** в **поле Учетная запись пользователя домена** щелкните **указать** , а затем укажите имя пользователя (**контосо\рмс**) и пароль () и нажмите кнопку <strong>pass@word1</strong> **ОК**, а затем нажмите кнопку **Далее**.
...
28. На странице **Результаты** нажмите кнопку **Закрыть**, а затем на странице **Ход выполнения установки** нажмите **Закрыть**. По завершении выйдите из системы и войдите в систему как контосо\рмс, используя предоставленный пароль ( <strong>pass@word1</strong> ).
...
34. Перейдите в папку C:\Inetpub\wwwroot \\ _wmcs \цертификатион, выберите файл ServerCertification. asmx и добавьте пользователей, прошедших проверку подлинности, чтобы иметь разрешения на чтение и запись в файл.
...
## <a name="lab-setup-for-deploying-claims-across-forests-scenario"></a><a name="BKMK_CF"></a>Настройка лаборатории для развертывания утверждений в нескольких лесах
...
1. Подключите виртуальную машину к сети ID_AD_Network. Войдите в DC2 от имени администратора с паролем <strong>Pass@word1</strong> .
...
### <a name="set-contosocom-as-a-trusting-forest-to-adatumcom"></a><a name="BKMK_2.22"></a>Настройка contoso.com как доверяющего леса для adatum.com
...
### <a name="create-the-company-claim-type-on-adataumcom"></a><a name="BKMK_2.5"></a>Создание типа утверждения "Компания" для adataum.com
...
### <a name="enable-the-company-resource-property-on-contosocom"></a><a name="BKMK_2.55"></a>Включение свойства ресурса "Компания" для contoso.com
...
### <a name="enable-dynamic-access-control-on-adatumcom"></a><a name="BKMK_2.6"></a>Включение динамического контроля доступа для adatum.com
...
### <a name="create-the-company-claim-type-on-contosocom"></a><a name="BKMK_2.8"></a>Создание типа утверждения "Компания" для contoso.com
...
### <a name="create-the-central-access-rule"></a><a name="BKMK_2.9"></a>Создание правила централизованного доступа
...
7. Нажмите кнопку **ОК** три раза, чтобы завершить операцию и вернуться в центр администрирования Active Directory.
   ...
   ![Руководство по решению. ](media/Appendix-B--Setting-Up-the-Test-Environment/PowerShellLogoSmall.gif) * *_<em>эквивалентные команды Windows PowerShell</em>_* _
...
### <a name="publish-the-new-policy-through-group-policy"></a><a name="BKMK_2.11"></a>Публикация новой политики с помощью групповой политики
...
1.  На экране _ *Start** введите **Администрирование** и в строке **поиска** щелкните **Параметры**. В результатах **Параметры** щелкните **Администрирование**. Откройте консоль управления групповой политикой в папке **Администрирование**.
...
1.  На **начальном** экране введите **Управление групповой политикой** в поле **Поиск**. Откройте **Управление групповой политикой** в папке "Администрирование".
    ...
    > [!TIP]
    > Если параметр **Показать средства администрирования** был выключен, то папка "Администрирование" и ее содержимое не будут показаны в результатах.
...
### <a name="create-the-earnings-folder-on-the-file-server"></a><a name="BKMK_2.12"></a>Создание папки Earnings на файловом сервере
Создайте новый том NTFS на сервере FILE1 и создайте следующую папку: D:\Earnings.

> [!NOTE]
> По умолчанию централизованные политики доступа не включены для системы или загрузочного тома C:.

### <a name="set-classification-and-apply-the-central-access-policy-on-the-earnings-folder"></a><a name="BKMK_2.13"></a>Настройка классификации и применение централизованной политики доступа к папке Earnings
...
1. В диспетчере Hyper-V подключитесь к серверу FILE1. Войдите на сервер с помощью Contoso\Administrator с паролем <strong>pass@word1</strong> .
...
5. Перейдите на вкладку **классификация** . Выберите **компания**, а затем в поле **значение** выберите **adatum** .
