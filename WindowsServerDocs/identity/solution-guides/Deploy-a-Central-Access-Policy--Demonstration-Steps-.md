---
description: Дополнительные сведения о развертывании централизованной политики доступа (демонстрационные шаги)
...
## <a name="set-up-a-test-environment"></a><a name="BKMK_1.1"></a>Настройка тестовой среды
...
## <a name="plan-identify-the-need-for-policy-and-the-configuration-required-for-deployment"></a><a name="BKMK_1.2"></a>План: Указание необходимости в политике и конфигурации, необходимой для развертывания
В этом разделе представлено высокоуровневое описание планирования развертывания.

|Номер шага | Шаг | Пример |
|--|--|--|
...
| 1,3 | Выразить политику доступа в конструкциях Windows Server 2012 | Цель:<p>-Resource. Department содержит финансовый отдел<p>Правила доступа:<p>— Разрешить чтение пользователя. страна = ресурс. страна и пользователь. штат = ресурс. Отдел<br />-Разрешить полный доступ User. MemberOf (Финанцеадмин)<p>Исключение:<p>Allow read memberOf(FinanceException) |  |
| 1.4 | Определение свойств файла, необходимых для политики | Отметьте файлы как:<p>— Отдел<br />-Country |  |
| 1.5 | Определение типов утверждений и групп, необходимых для политики | Типы утверждений:<p>-Country<br />— Отдел<p>Группы пользователей<p>-Финанцеадмин<br />-Финанцеексцептион |  |
| 1.6 | Определение серверов, к которым следует применить эту политику | Примените политику. |  |

## <a name="implement-configure-the-components-and-policy"></a><a name="BKMK_1.3"></a>Реализация: Настройка компонентов и политики
В этом разделе представлен пример, который развертывает централизованную политику доступа для финансовых документов.

| Номер шага | Шаг | Пример |
|--|--|--|
| 2.1 | Создание типов утверждений | Создайте следующие типы утверждений:<p>— Отдел<br />-Country |
| 2.2 | Создание свойства ресурсов | Создайте и включите следующие свойства ресурсов:<p>— Отдел<br />-Country |
...
![Руководство по решению. ](media/Deploy-a-Central-Access-Policy--Demonstration-Steps-/PowerShellLogoSmall.gif) * *_<em>эквивалентные команды Windows PowerShell</em>_* _
...
1.  В левой области центр администрирования Active Directory щелкните значок _ * представление в виде дерева * *. Разверните узел **Динамический контроль доступа** и выберите **Свойства ресурса**.
...
1. В левой области центр администрирования Active Directory щелкните _ * представление в виде дерева * *, выберите **динамический контроль доступа**, а затем выберите **Централизованные правила доступа**.
...
4. В разделе **Целевые ресурсы** нажмите кнопку **Изменить**, а затем в диалогом окне **Централизованное правило доступа** щелкните **Добавить условие**. Добавьте следующее условие: [**ресурс**] [**отдел**] [**равно**] [**значение**] [**Finance**], а затем нажмите кнопку **ОК**.
...
7. В диалоговом окне **запись разрешения для разрешений** щелкните **Добавить условие** и добавьте следующие условия: [**пользователь**] [**страна**] [**любой из**] [**ресурс**] [**Country**] щелкните **Добавить условие**.
    [**И**] . Щелкните [**Пользователь**] [**Отдел**] [**Любой из**] [**Ресурс**] [**Отдел**]. Задайте для параметра **Разрешения** значение **Чтение**.
...
1. В левой области центр администрирования Active Directory щелкните значок _ * динамический контроль доступа * *, а затем выберите пункт **политики централизованного доступа**.
...
1.  На экране _ *Start** в поле **поиска** введите **Групповая политика управление**. Дважды щелкните **Управление групповой политикой**.
...
## <a name="deploy-the-central-access-policy"></a><a name="BKMK_1.4"></a>Развертывание централизованной политики доступа
...
3. Вам также необходимо обновить глобальные свойства ресурсов из Active Directory. Откройте командное окно Windows PowerShell с повышенными привилегиями и введите: `Update-FSRMClassificationpropertyDefinition`. Нажмите клавишу ВВОД и закройте Windows PowerShell.
   ...
   >
   > 1. Войдите в файл файла FILE1 как CONTOSO\Administrator, используя пароль <strong>pass@word1</strong> .
...
7. На вкладке **Безопасность** нажмите кнопку **Дополнительно**. Щелкните вкладку **Централизованная политика**.
...
3.  Чтобы проверить разрешения для пользователя, щелкните **выбрать пользователя**, введите имя пользователя, а затем щелкните  **Просмотр действующих прав доступа** , чтобы просмотреть действующие права доступа. Пример.
...
## <a name="maintain-change-and-stage-the-policy"></a><a name="BKMK_1.5"></a>Обслуживание: изменение и размещение политики
...
8. Щелкните ссылку **Добавить условие** еще раз и добавьте следующее условие:
   Перетаскивани [**Пользователь**] [**Группа**] [**Член любого**] [**Значение**] \( **Финанцеексцептион**)
...
2. Откройте окно командной строки и введите _ * gpupdate/force * *. Эта команда применяет изменения групповой политики на сервере.
...
4. Дважды щелкните ярлык на рабочем столе, чтобы \\ \FILE1\Finance документы. У пользователя EValle по-прежнему должен быть доступ к файлам. Переключитесь на сервер FILE1.
