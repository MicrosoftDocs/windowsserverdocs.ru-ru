...
5. (Необязательно.) Обеспечьте подключение к облачным службам. Этот шаг позволяет настроить управление облачными службами, например Azure и Office 365, с соответствующим уровнем безопасности. Кроме того, этот шаг необходимо выполнить для обеспечения управления рабочими станциями с привилегированным доступом с помощью Microsoft Intune.
   ...
   * Для обеспечения подключения к службам рабочей станции сделайте следующее:
   ...
   2. Примените базовые конфигурации безопасности Windows 10 и ссылку доступа к облачной службе — базовые конфигураций безопасности для Windows и доступа к облачным службам (при необходимости) — к соответствующим подразделениям, выполнив следующее:
      ...
      2. Создайте объекты групповой политики ниже, [импортируйте параметры политики](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753786(v=ws.11)) и [выполните привязку](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc732979(v=ws.11)) в соответствии с этой таблицей. Привяжите политики к расположениям в порядке, приведенном в таблице (приоритетные записи в таблице ниже следует применить позже):
...
6. (Необязательно.) Установите дополнительные средства, необходимые для администраторов уровня 1. Установите любые средства и скрипты, необходимые для выполнения рабочих обязанностей. Оцените риск раскрытия учетных данных на целевых компьютерах с помощью любого инструмента, прежде чем добавлять их в рабочую станцию с привилегированным доступом. Дополнительные сведения об оценке риска раскрытия учетных данных для средств администрирования и способов подключения см. на [этой странице](./securing-privileged-access-reference-material.md). Получите все установочные носители в соответствии с указаниями в разделе о принципе чистоты источника для установочных носителей.
...
1. **Включите многофакторную проверку подлинности для привилегированных учетных записей**.  Многофакторная идентификация повышает безопасность учетной записи пользователя, так как пользователь должен указывать учетные данные и физический маркер.  Многофакторная идентификация эффективно дополняет политики идентификации. При развертывании она не зависит от этих политик (это утверждение верно и в обратном порядке).  Корпорация Майкрософт рекомендует использовать одну из этих форм многофакторной идентификации:
   ...
   * **Windows Hello для бизнеса**: Windows Hello для бизнеса позволяет пользователю проводить проверку подлинности своей учетной записи Майкрософт, учетной записи Active Directory, учетной записи Microsoft Azure Active Directory или сторонней службы (не Майкрософт), которая поддерживает проверку подлинности Fast ID Online (FIDO). После начальной двухэтапной проверки при регистрации в Windows Hello для бизнеса на устройстве пользователя настраивается служба Windows Hello для бизнеса и пользователь определяет жест, который может быть Windows Hello или PIN-кодом. Учетные данные Windows Hello для бизнеса — это пара асимметричных ключей, которые можно сгенерировать в изолированных средах доверенных платформенных модулей (TPM).
      Дополнительные сведения о Windows Hello для бизнеса см. в [этой статье](/windows/security/identity-protection/hello-for-business/hello-identity-verification).
...
Эксплуатация решения рабочей станции с привилегированным доступом осуществляется согласно стандартам в разделе [Рабочие стандарты на основе принципа чистоты источника](./securing-privileged-access-reference-material.md).
...
[Защищенная структура](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms-top-node.md) может использоваться для запуска рабочих нагрузок рабочих станций с привилегированным доступом в экранированной виртуальной машине на ноутбуке или на сервере перехода.
...
Полное описание топологии защищенной структуры и сведения о безопасности см. в документации по [защищенной структуре](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms-top-node.md).
...
Запускать экранированные виртуальные машины могут только те компьютеры, которые известны службе HGS и запускаются с помощью доверенной [политики целостности кода](/windows/security/threat-protection/windows-defender-application-control/windows-defender-application-control).
...
[Защита извлеченных учетных данных домена с помощью Credential Guard](/windows/security/identity-protection/credential-guard/credential-guard)

[Обзор Device Guard](/windows/security/threat-protection/device-guard/introduction-to-device-guard-virtualization-based-security-and-windows-defender-application-control)

[Protecting high-value assets with secure admin workstations](/previous-versions/mt186538(v=technet.10)) (Защита ценных ресурсов с помощью защищенных рабочих станций администрирования)
...
[What's New in Kerberos Authentication](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/hh831747(v=ws.11)) (Что нового в проверке подлинности Kerberos для Windows Server 2012)
...
[Обзор технологии доверенного платформенного модуля](/windows/device-security/tpm/trusted-platform-module-overview)