...
![Поток On-Behalf-Of](media/adfs-scenarios-for-developers/obo.png)
  ...
  1. Клиентское приложение выполняет запрос к API А с маркером А. Примечание. Настраивая в AD FS поток On-Behalf-Of, убедитесь, что выбрана область `user_impersonation` и в запросе от клиента указана именно область `user_impersonation`.
