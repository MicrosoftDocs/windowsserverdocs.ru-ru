...
2. Выберите Библиотека классов и убедитесь, что вы используете .NET 4,5.
    ...
    ![Снимок экрана: диалоговое окно "новый проект" с выбранным параметром "Библиотека классов".](media/ad-fs-build-custom-auth-method/Dn783423.71a57ae1-d53d-462b-a846-5b3c02c7d3f2(MSDN.10).jpg "создание поставщика")
...
6. Нажмите кнопку **ОК** , чтобы подтвердить новую ссылку:
    ...
    ![Снимок экрана: диалоговое окно диспетчера ссылок, в котором отображается выбранная Microsoft.IdentityServer.Web.dll.](media/ad-fs-build-custom-auth-method/Dn783423.f18df353-9259-4744-b4b6-dd780ce90951(MSDN.10).jpg "создание поставщика")
...
7. Добавьте в проект новый класс (щелкните проект правой кнопкой мыши, **добавьте... Класс...**) и присвойте ему имя, например **мядаптер**, как показано ниже:
    ...
    ![Снимок экрана: диалоговое окно "Добавление нового элемента" с выбранным параметром "класс".](media/ad-fs-build-custom-auth-method/Dn783423.6b6a7a8b-9d66-40c7-8a86-a2e3b9e14d09(MSDN.10).jpg "создание поставщика")
...
9. Мы еще не готовы к сборке... Есть еще два интерфейса для перехода.
...
10. Далее можно добавить необходимые элементы для каждого из них. Во-первых, метаданные (с полезными встроенными комментариями)
...
11. Замените все содержимое класса следующим:
...
12. Мы еще не готовы к сборке... Есть еще два интерфейса для перехода.
...
13. Далее можно добавить необходимые элементы для каждого из них. Во-первых, метаданные (с полезными встроенными комментариями)
...
14. Обратите внимание на "TODO" для элемента **Resources. формпажехтмл** выше. Вы можете исправить это в минуту, но сначала добавим окончательные обязательные инструкции Return, основанные на вновь реализованных типах, в исходный класс Мядаптер. Для этого добавьте следующий объект в имеющуюся реализацию Иаусентикатионадаптер:
...
15. Теперь для файла ресурсов, содержащего фрагмент HTML. Создайте в папке проекта новый текстовый файл со следующим содержимым:
...
16. Затем выберите **проект — \> Добавить компонент... Файл ресурсов** и назовите файловые **ресурсы** и нажмите кнопку **Добавить:**

    ![Снимок экрана: диалоговое окно "Добавление нового элемента" с выбранным файлом ресурсов.](media/ad-fs-build-custom-auth-method/Dn783423.3369ad8f-f65f-4f36-a6d5-6a3edbc1911a(MSDN.10).jpg "создание поставщика")

17. Затем в файле **Resources. resx** выберите **Добавить ресурс... Добавить существующий файл**. Перейдите к текстовому файлу (содержащему фрагмент HTML), сохраненному ранее.
...
4. В разделе **Выбор дополнительных методов проверки подлинности** в нижней части страницы установите флажок для админнаме поставщика. Нажмите кнопку **Применить**.
...
1. Сначала включите поставщик в глобальной политике:
    ...
    > [!NOTE]
    > Обратите внимание, что значение, указанное для параметра Аддитионалаусентикатионпровидер, соответствует значению, указанному для параметра Name в приведенном выше командлете Register-AdfsAuthenticationProvider, и свойству Name из выходных данных командлета Get-AdfsAuthenticationProvider.
...
5. Откроется страница форм MFA с примерами вопросов.
    ...
    ![Снимок экрана страницы M F A Forms с примерами вопросов.](media/ad-fs-build-custom-auth-method/Dn783423.c98d2712-cbd3-4cb9-ac03-2838b81c4f63(MSDN.10).jpg "проверка подлинности с помощью адаптера")

    ![Снимок экрана со страницей выбора M F A.](media/ad-fs-build-custom-auth-method/Dn783423.fd3aefc0-ef6c-4a8c-a737-4914c78ff2d2(MSDN.10).jpg "проверка подлинности с помощью адаптера")

Теперь у вас есть рабочая реализация интерфейса, и вы имеете знания о том, как работает модель. Вы можете воспользоваться дополнительным примером для установки точек останова в Бегинаусентикатион, а также Трендаусентикатион. Обратите внимание на то, как Бегинаусентикатион выполняется при первом входе пользователя в форму MFA, в то время как Трендаусентикатион активируется при каждой отправке формы.
...
Обратите внимание, что передаваемое "имя" значение совпадает с именем "имя", предоставленным командлету Register-AdfsAuthenticationProvider. Это также свойство Name, которое выводится из Get-Адфсаусентикатионпровидер.
...
4. В разделе **Выбор дополнительных методов проверки подлинности** установите флажок для админнаме поставщика. Нажмите кнопку **Применить**.
...
Вы должны увидеть успешный вход при вводе _адфабрик_ на странице проверки подлинности mfa.

![Снимок экрана страницы M F A Forms с примером текста запроса.](media/ad-fs-build-custom-auth-method/Dn783423.630d8a91-3bfe-4cba-8acf-03eae21530ee(MSDN.10).jpg "Выполните вход с использованием адаптера")

![Снимок экрана со страницей "M F" успешного входа.](media/ad-fs-build-custom-auth-method/Dn783423.c340fa73-f70f-4870-b8dd-07900fea4469(MSDN.10).jpg "Выполните вход с использованием адаптера")
