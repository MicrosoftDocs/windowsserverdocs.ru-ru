...
Командлет **Set-адфссинкпропертиес** изменяет частоту синхронизации для базы данных конфигурации службы федерации Active Directory (AD FS) (AD FS).
...
Эта команда изменяет основной сервер AD FS в ферме WID на сервер-получатель. Необходимо указать полное доменное имя основного сервера. Если этого не сделать, то, возможно, не все серверы-получатели AD FS синхронизироваться должным образом. Примечание. основной сервер должен быть доступен по протоколу HTTP через порт 80 с сервера-получателя.

Дополнительные сведения см. в разделе [Set-адфссинкпропертиес](https://docs.microsoft.com/en-us/powershell/module/adfs/set-adfssyncproperties?view=win10-ps) .
