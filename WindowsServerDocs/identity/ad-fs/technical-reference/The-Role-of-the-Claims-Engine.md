...
Как показано на следующем рисунке, действие принятия входящих правил принятия утверждений \( \) , авторизация правил авторизации запрашивающих утверждений \( и предоставление \) исходящих \( правил выдачи утверждений \) через правила утверждений для всех федеративных отношений доверия в организации выполняется подсистемой утверждений.

![Иллюстрация, показывающая процессы, выполняемые подсистемой утверждений.](media/adfs2_enginepipeline.gif)

## <a name="claim-rules-execution-process"></a>Процесс выполнения правил утверждений
При настройке отношения доверия поставщика утверждений или отношения доверия с проверяющей стороной в Организации с правилами утверждений набор правил утверждений \( \) для этого отношения доверия действует как привратник для входящих утверждений, вызывая подсистему утверждений для применения необходимой логики в правилах утверждений, чтобы определить, следует ли выдавать заявки и какие утверждения должны выдаваться.

В следующем подразделе описывается каждое из действий, которое выполняется модулем в потоке выполнения правил утверждения. Каждое из описанных действий производится на каждом этапе процесса конвейерной обработки утверждений. А именно:
...
![Иллюстрация, показывающая, что обработчик утверждений считывает утверждения A и B из входящих утверждений и копирует их в набор входящих утверждений.](media/adfs2_context1.gif)
...
Согласно языку правил утверждений, правило утверждения состоит из двух частей: условия и инструкции выдачи. Подсистема утверждений сначала обрабатывает условие с помощью данных в наборе входящих утверждений, чтобы определить, имеет ли условие, заданное в правиле, значение true для утверждений, содержащихся во входном утверждении \( . набор утверждений, которые соответствуют условию правила, называется соответствующими утверждениями \) . Если найдены соответствующие утверждения, модуль утверждений выполняет инструкцию выдачи правила для каждого набора соответствующих утверждений. Инструкция выдачи правила может выполнять одно из следующих действий с соответствующими утверждениями:
...
![Иллюстрация, показывающая, что происходит при выполнении обработчиком утверждений Правила преобразования.](media/adfs2_context2.gif)
...
Если набор правил утверждения, выполняемый на шаге 2 процесса выполнения правил утверждений, состоит из правил авторизации, \( которые имеют разные наборы входных и выходных утверждений, чем правила принятия или выдачи \) , то эти правила авторизации будут выполняться, чтобы определить, авторизован ли инициатор запроса для получения маркера безопасности для данной проверяющей стороны из Служба федерации на основе утверждений запросившего.

Целью правил авторизации является выдача разрешающего или запрещающего утверждения в зависимости от того, следует ли разрешить пользователю получить токен для данной проверяющей стороны или нет. Как показано на следующем рисунке, выходные данные выполнения авторизации используются конвейером для определения того, выполняется ли набор правил выдачи или нет — на основе присутствия или отсутствия утверждения разрешения и \/ запрета, но выходные данные выполнения авторизации не используются в качестве входных данных для набора правил утверждений.

![Иллюстрация, показывающая выходные данные выполнения авторизации, используется конвейером для определения того, выполняется ли набор правил выдачи.](media/adfs2_authorization.gif)
