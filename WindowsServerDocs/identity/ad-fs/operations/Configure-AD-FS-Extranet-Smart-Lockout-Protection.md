...
title: Настройка защиты интеллектуальной блокировки экстрасети AD FS
description: 'Дополнительные сведения: AD FS блокировки экстрасети и Smart-блокировки экстрасети'
author: billmath
ms.author: billmath
manager: mtilman
ms.date: 05/20/2019
...
![Схема рабочего процесса, на которой показан неудачный процесс входа.](media/configure-ad-fs-extranet-smart-lockout-protection/esl2.png)
...
```
### <a name="enable-log-only-mode"></a>Включить режим Log-Only
...
В AD FS 2016, если перед включением Smart-блокировки экстрасети включено поведение 2012 R2 "Мягкая блокировка экстрасети", Log-Only режим отключит поведение "Мягкая блокировка экстрасети". AD FS интеллектуальная блокировка не будет блокировать пользователей в режиме Log-Only. Однако локальная служба AD может блокировать пользователя на основе конфигурации AD. Проверьте политики блокировки AD, чтобы узнать, как локальная служба AD может заблокировать пользователей.
...
В режиме "только журнал" можно проверить журнал аудита безопасности на наличие событий блокировки. Для всех обнаруженных событий можно проверить пользовательское состояние с помощью командлета Get-ADFSAccountActivity, чтобы определить, произошла ли блокировка от знакомых или незнакомых IP-адресов, и дважды проверить список известных IP-адресов для этого пользователя.
...
|562 (ADFS 2019)|Произошла ошибка при обмене данными с конечной точкой хранилища учетных записей на сервере %1.</br>Сообщение об исключении: %2|
