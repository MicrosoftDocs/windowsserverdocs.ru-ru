---
description: Дополнительные сведения о настройке альтернативного имени для входа
...
или см. Дополнительные сведения [о гибридной современной проверке подлинности](https://support.office.com/article/Hybrid-Modern-Authentication-overview-and-prerequisites-for-using-it-with-on-premises-Skype-for-Business-and-Exchange-servers-ef753b32-7251-4c9e-b442-1a5aec14e58d)
...
1. ответ. пользователь подготавливается в Azure AD с помощью альтернативного идентификатора.
   </br>б. Администратор каталогов принудительно отправит требуемые параметры RegKey на затронутые клиентские компьютеры.
...
|Клиент|Условия поддержки|Примечания|
| ----- | -----|-----|
...
| ----- |----- | ----- |
|Outlook|Поддерживается, лишних запросов нет|Поддерживается</br></br>С **современной проверкой подлинности** для Exchange Online: поддерживается</br></br>С **обычной проверкой подлинности** для Exchange Online: поддерживается с учетом следующих предостережений:</br><li>Необходимо находиться на компьютере, присоединенном к домену, и быть подключенным к корпоративной сети. </li><li>Альтернативный идентификатор можно использовать только в средах, которые не допускают внешний доступ для пользователей почтовых ящиков. Это означает, что пользователи могут проходить проверку подлинности в почтовом ящике поддерживаемым образом, если они подключены и присоединены к корпоративной сети, VPN или подключены через компьютеры с прямым доступом, но при настройке профиля Outlook вы получаете несколько дополнительных запросов.|
|Гибридные общие папки|Поддерживается, дополнительные запросы отсутствуют.|С **современной проверкой подлинности** для Exchange Online: поддерживается</br></br>С **обычной проверкой подлинности** для Exchange Online: не поддерживается</br></br><li>Гибридные общие папки не могут расширяться, если используются альтернативные ИДЕНТИФИКАТОРы, поэтому их не следует использовать в сегодняшних случаях с обычными методами проверки подлинности.|
|Кросс — локальное делегирование|См. раздел [Настройка Exchange для поддержки делегированных разрешений почтового ящика в гибридном развертывании](/exchange/hybrid-deployment/set-up-delegated-mailbox-permissions) .|См. раздел [Настройка Exchange для поддержки делегированных разрешений почтового ящика в гибридном развертывании](/exchange/hybrid-deployment/set-up-delegated-mailbox-permissions) .|
