...
Когда AD FS получает запрос на приложение, AD FS получает приложение из SQL и добавляет его в кэш. В `cacheRefreshIntervalSecs` значении приложение в кэше обновляется с помощью фонового потока. Хотя запись существует в кэше, входящие запросы будут использовать кэш во время фонового обновления. Если к записи нет доступа в течение 5 * `cacheRefreshIntervalSecs` , она удаляется из кэша. Самая старая запись также может быть удалена из кэша после достижения настраиваемого `maxRelyingPartyEntries` значения.

>[!NOTE]
> Данные кэша будут обновляться вне `cacheRefreshIntervalSecs` значения, если ADFS получает уведомление от SQL, что означает, что в базе данных произошло изменение. Это уведомление вызывает обновление кэша.
...
Дополнительные поддерживаемые настраиваемые значения:
   ...
   - **максклиентентриес** — максимальное число записей клиента OAuth, AD FS будет оставаться в памяти. Значение по умолчанию — 500.
   - **максклаимдескрипторентриес** — максимальное число записей дескриптора утверждений, AD FS будет оставаться в памяти. Значение по умолчанию — 500.
...
Перед настройкой поддержки нескольких артефактов для базы данных запустите обновление на всех узлах и обновите двоичные файлы, так как в этой функции происходят вызовы с несколькими узлами.
 ...
 Например, чтобы создать скрипт развертывания только для базы данных артефактов, укажите `-DatabaseType` параметр и передайте значение "артефакт". Необязательный `-DatabaseType` параметр указывает тип базы данных AD FS и может принимать значение: ALL (по умолчанию), артефакт или Конфигурация. Если `-DatabaseType` параметр не указан, скрипт настроит как артефакт, так и скрипт конфигурации.
...
Созданный скрипт должен выполняться на компьютере SQL для создания необходимых баз данных и предоставления AD FS учетной записи службы, разрешения SA SQL для этих баз данных.
 ...
 >[!NOTE]
 > Артифактсторе и connectionString учитывают регистр значений. Убедитесь, что они настроены правильно.
 &lt;Артифактсторе connectionString = "источник данных = .\Склинстанце; встроенная безопасность = true; начальный каталог = Адфсартифактсторе"/&gt;
>
