---
description: 'Дополнительные сведения: Настройка лабораторной среды AD FS'
...
-   [Пошаговое руководство. Управление рисками с помощью дополнительной многофакторной проверки подлинности для конфиденциальных приложений](Walkthrough-Guide--Manage-Risk-with-Additional-Multi-Factor-Authentication-for-Sensitive-Applications.md)
...
Сведения о создании учетных записей пользователей и групп в Active Directory (AD) см. в разделе [https://technet.microsoft.com/library/cc783323%28v=ws.10%29.aspx](/previous-versions/windows/it-pro/windows-server-2003/cc783323(v=ws.10)) .
...
1.  > [!IMPORTANT]
    ...
    При появлении запроса на ввод учетной записи службы введите **контософсгмса $**.
...
2.  [Установка набора инструментов Windows Identity Foundation SDK](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_13)
...
10. На странице **Подтверждение выбранных элементов для установки** щелкните **Указать альтернативный исходный путь**. Введите путь к каталогу SxS, который находится на установочном носителе Windows Server 2012 R2. Например, Д:саурцессксс. Нажмите кнопку **ОК**, а затем кнопку **Установить**.
...
4.  **Настройка диспетчера IIS Manager**
    ...
    7.  Задайте для псевдонима **клаимапп** и физический путь к **к:инетпубклаимапп**.

5.  Для настройки **claimapp** для работы с сервером федерации выполните следующие действия.

    1.  Запустите FedUtil.exe, который находится в **файлах c:Program Files (x86) Windows Identity Foundation сдкв 3.5**.

    2.  Задайте для расположения конфигурации приложения значение **C:inetputclaimappweb.config** и задайте для URI приложения URL-адрес сайта **https://webserv1.contoso.com /клаимапп/**. Нажмите кнопку **Далее**.
...
[Службы федерации Active Directory (AD FS) How-To серии видео: Установка фермы](https://channel9.msdn.com/Search?term=Active%20Directory%20Federation%20Services#pubDate=year&ch9Search) 
 серверов AD FS [Службы федерации Active Directory (AD FS) How-To серии видеоматериалов: обновление сертификатов](https://channel9.msdn.com/Search?term=Active%20Directory%20Federation%20Services#pubDate=year&ch9Search) 
 [Службы федерации Active Directory (AD FS) How-To серии видео: Добавление отношения доверия с проверяющей стороной](https://channel9.msdn.com/Search?term=Active%20Directory%20Federation%20Services#pubDate=year&ch9Search) 
 [Службы федерации Active Directory (AD FS) How-To серии видео: Включение службы](https://channel9.msdn.com/) 
 регистрации устройств [Серия Видеороликов службы федерации Active Directory (AD FS) How-To: Установка прокси-службы веб приложения](https://channel9.msdn.com/Search?term=Active%20Directory%20Federation%20Services#pubDate=year&ch9Search)
