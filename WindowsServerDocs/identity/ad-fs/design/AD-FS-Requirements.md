...
- Встроенная проверка подлинности Windows с помощью Negotiate (только NTLM) для конечных точек WS-Trust, которые принимают встроенную проверку подлинности Windows
...
|AES256[http://www.w3.org/2001/04/xmlenc#aes256-cbc](http://www.w3.org/2001/04/xmlenc#aes256-cbc)|256|**По умолчанию**. Поддерживаемый алгоритм шифрования маркера безопасности.|
...
|SHA1[http://www.w3.org/PICS/DSig/SHA1_1_0.html](http://www.w3.org/PICS/DSig/SHA1_1_0.html)|Н/Д|Используется сервером AD FS в процессе создания SourceId артефакта: в этом сценарии STS использует SHA1 (согласно рекомендациям в стандарте SAML 2,0), чтобы создать короткое значение 160 для sourceiD артефакта.<p>Также используется веб-агентом ADFS (устаревший компонент из WS2003) для идентификации изменений в значении времени последнего обновления, чтобы оно знало, когда следует обновлять сведения от STS.|
|SHA1withRSA<p>[http://www.w3.org/PICS/DSig/RSA-SHA1_1_0.html](http://www.w3.org/PICS/DSig/RSA-SHA1_1_0.html)|Н/Д|Используется в случаях, когда AD FS Server проверяет подпись SAML Аусентикатионрекуест, подписывает запрос на разрешение артефакта или ответ, создает сертификат подписи маркера.<p>В таких случаях SHA256 используется по умолчанию, а SHA1 используется только в том случае, если партнер (проверяющая сторона) не поддерживает SHA256 и должен использовать SHA1.|
