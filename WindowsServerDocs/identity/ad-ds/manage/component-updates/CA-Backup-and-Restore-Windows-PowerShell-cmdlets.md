...
|-Киплог|1. указывает команде на необходимость сохранения файлов журнала. Если параметр не указан, файлы журнала усекаются по умолчанию, за исключением случая добавочного сценария.<p>Backup-Каролесервице c:\adcsbackup7-Киплог|
...
|-Password|— Указывает пароль сертификатов ЦС и закрытых ключей.<br />— Должна быть защищенной строкой<p>**Пример**.<p>Restore-Каролесервице c:\adcsbackup4-Password (чтение-хост-запрос "пароль:"-AsSecureString)-Force<p>Restore-Каролесервице c:\adcsbackup5-Password (ConvertTo-SecureString "Pa55w0rd!" -AsPlainText-Force) — Force|
|-Датабасеонли|Восстановление базы данных без сертификата ЦС<p>Restore-Каролесервице c:\adcsbackup6-Датабасеонли|
|-Force|— Позволяет перезаписать существующие ключи<br />— Необязательный параметр, но при восстановлении на месте, скорее всего, требуется<p>Restore-Каролесервице c:\adcsbackup1-Force|
...
|----------|---------|-----------|
|**Restore-Каролесервице К:\адксбаккуп**|Restore-Каролесервице: процесс не может получить доступ к файлу, так как он используется другим процессом. (Exception from HRESULT:<p>0x80070020|Перед запуском командлета Restore-Каролесервице завершите работу службы служб сертификатов Active Directory|
