---
description: Дополнительные сведения о аттестации ключей TPM
...
1.  Вкладка **Совместимость**
    ...
    ![Снимок экрана, в котором выделяется список получателей сертификата.](media/TPM-Key-Attestation/GTR_ADDS_CompatibilityTab.gif)
...
2.  Вкладка **Шифрование**
    ...
    ![Снимок экрана, на котором показаны списки категорий поставщиков и имен алгоритмов.](media/TPM-Key-Attestation/GTR_ADDS_CryptoTab.gif)

3.  Вкладка **аттестации ключей**

    Это новая вкладка для Windows Server 2012 R2:

    ![Снимок экрана, на котором показана вкладка аттестации ключей.](media/TPM-Key-Attestation/GTR_ADDS_ConfigCertTemplate.gif)

    Выберите режим аттестации из трех возможных параметров.

    ![Снимок экрана, на котором показаны режимы аттестации.](media/TPM-Key-Attestation/GTR_ADDS_KeyModes.gif)
    ...
    -   **Требуется:** Клиент *должен* выполнить аттестацию ключа ДОВЕРЕНного платформенного модуля, в противном случае запрос на сертификат завершится ошибкой.

    Затем выберите модель доверия TPM. Еще три варианта:

    ![Снимок экрана, на котором показаны модели доверия TPM.](media/TPM-Key-Attestation/GTR_ADDS_KeyTypeToEnforce.gif)
...
1.  Для проверки доверия в Екпуб выполните следующие два действия.
    ...
    2.  **Проверка доверия в екцерт на компьютере ЦС:** Скопируйте извлеченную строку (хэш SHA-2 Екпуб) на сервер (например, через электронную почту) и передайте ее в командлет Confirm-CAEndorsementKeyInfo. Обратите внимание, что этот параметр должен содержать 64 символов.
...
[Обзор](/previous-versions/windows/it-pro/windows-8.1-and-8/jj131725(v=ws.11)) 
 технологии доверенный платформенный модуль (TPM) [Внешний ресурс: доверенный платформенный модуль (TPM)](http://www.cs.unh.edu/~it666/reading_list/Hardware/tpm_fundamentals.pdf)
