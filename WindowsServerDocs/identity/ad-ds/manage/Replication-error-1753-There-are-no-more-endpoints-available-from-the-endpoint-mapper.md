...
Текст заголовка диалогового окна: диалоговое окно "Репликация" текст сообщения: при попытке синхронизации контекста именования \<%directory partition name%> из контроллера домена с контроллером домена произошла следующая ошибка \<Source DC> \<Destination DC> :
...
Все контроллеры домена в общем лесу Active Directory регистрируют запись CNAME контроллера домена в _msdcs. \<forest root domain> Зона DNS независимо от домена, в котором они находятся в пределах леса. Запись CNAME контроллера домена является производной от атрибута objectGUID объекта параметров NTDS для каждого контроллера доменов.
...
* Убедитесь, что значение запуска и состояние службы для службы RPC и локатора RPC указаны правильно для версии ОС клиента RPC (конечный контроллер домена) и RPC-сервера (исходный контроллер домена). Если служба в данный момент остановлена или не была настроена с использованием значений запуска по умолчанию, сбросьте значения параметров запуска по умолчанию, перезагрузите измененный контроллер домена и повторите операцию.
   * Кроме того, убедитесь, что контекст службы соответствует параметрам по умолчанию, перечисленным в следующей таблице.
      ...
      | Служба | Состояние по умолчанию (тип запуска) в Windows Server 2003 и более поздних версиях | Состояние по умолчанию (тип запуска) в Windows Server 2000 |
      | --- | --- | --- |
...
* [Как работает RPC](/windows/win32/rpc/how-rpc-works)
* [Как сервер готовится к подключению](/windows/win32/rpc/how-the-server-prepares-for-a-connection)
* [Как клиент устанавливает подключение](/windows/win32/rpc/how-the-client-establishes-a-connection)
* [Регистрация интерфейса](/windows/win32/rpc/registering-the-interface)
* [Обеспечение доступности сервера в сети](/windows/win32/rpc/making-the-server-available-on-the-network)
* [Регистрация конечных точек](/windows/win32/rpc/registering-endpoints)
* [Ожидание вызовов клиента](/windows/win32/rpc/listening-for-client-calls)
* [Как клиент устанавливает подключение](/windows/win32/rpc/how-the-client-establishes-a-connection)
...
* [Имя участника-службы для целевого контроллера домена в AD DS](/openspecs/windows_protocols/ms-drsr/41efc56e-0007-4e88-bafe-d7af61efd91f)
