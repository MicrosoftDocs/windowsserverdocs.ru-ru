---
description: Дополнительные сведения см. в статье Развертывание и настройка виртуализированного контроллера домена.
...
- [Безопасное восстановление виртуализированного контроллера домена](../../../ad-ds/get-started/virtual-dc/Virtualized-Domain-Controller-Deployment-and-Configuration.md#BKMK_VDCSafeRestore)
...
2. Клонирующий контроллер домена обращается к PDCE напрямую, используя протокол RPC DRSUAPI, чтобы создать объекты-компьютеры для клонированного контроллера домена.
    ...
    >
    > Дополнительные сведения см. в разделе [4.1.29 IDL_DRSAddCloneDC (Opnum 28)](/openspecs/windows_protocols/ms-drsr/ef0bfb1d-037b-4626-a6d9-cc7589bc5786).

**_Это также означает, что при использовании неполностью маршрутизируемых сетей клонирование виртуального контроллера домена требует наличия сегментов сети с доступом к пдце_* _. После клонирования клонированный контроллер домена можно переместить в другую сеть — как физический контроллер домена — при условии, что вы тщательно обновите информацию логического сайта доменных служб Active Directory.
...
1. С помощью оснастки DSA. msc щелкните домен правой кнопкой мыши и выберите команду _ * хозяева операций * *. Запомните контроллер домена, указанный на вкладке PDC, и закройте диалоговое окно.
...
|**ActiveDirectory**<p>**Командлет**|**Аргументы**|**Пояснение**|
|--|--|--|
|**New-ADDCCloneConfigFile**|*<no argument specified>*|Создает пустой файл DcCloneConfig.xml в рабочем каталоге DSA (расположение по умолчанию: %systemroot%\ntds).|
...
>
> **Get-VM-VMName** **_— Тестовая виртуальная машина_ | Get-VMNetworkAdapter | FL\\***
...
>
> [Принудительное выполнение заслуживающей и не заслуживающей доверия синхронизации для SYSVOL, реплицированного службой DFSR (как "D4/D2" для FRS)](https://support.microsoft.com/kb/2218556)
...
Например (с выходом, измененным для удобочитаемости и важными записями **_курсивом_*), здесь рассматривается партнерство репликации DC4:
...
Кроме того, если вы не знали, какие объекты не были реплицированы и только такоми, что все объекты были недоступными, можно использовать параметр _ */Статистикс**:
