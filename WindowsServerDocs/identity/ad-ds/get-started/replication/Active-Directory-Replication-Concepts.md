---
description: 'Дополнительные сведения: Active Directory основные понятия репликации'
...
Средство проверки согласованности знаний (KCC) автоматически создает объекты подключения, но их также можно создать вручную. Объекты подключения, созданные КСС, отображаются в оснастке "сайты и службы" Active Directory "как **<automatically generated>** и" при нормальных условиях работы. Объекты соединения, созданные администратором, создаются вручную. Объект подключения, созданный вручную, определяется по имени, назначенному администратором при его создании. При изменении **<automatically generated>** объекта соединения он преобразуется в административно измененный объект подключения, а объект отображается в виде идентификатора GUID. KCC не вносит изменения в ручные или измененные объекты подключения.
...
Для контроллеров домена Windows Server 2003 можно перераспределить рабочую нагрузку с помощью таких средств, как Adlb.exe, из каталога Windows Server 2003 Branch филиал Guide ( [https://go.microsoft.com/fwlink/?LinkID=28523](https://go.microsoft.com/fwlink/?LinkID=28523) ).

Для Windows Server 2008 RODC нормальная работа KCC обеспечивает некоторую перераспределение, что устраняет необходимость использовать дополнительное средство, например Adlb.exe. Новые функции включены по умолчанию. Его можно отключить, добавив следующий набор разделов реестра на RODC:
...
## <a name="subnet"></a><a name="BKMK_4"></a>Подсеть
...
## <a name="site"></a><a name="BKMK_5"></a>Сайт
