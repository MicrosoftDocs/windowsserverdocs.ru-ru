...
- **Отработка отказа**. при возникновении сбоя в первичном (или дополнительном) расположении можно вручную инициировать тестовую, плановую или внеплановую отработку отказа.
    ...
    |--|--|--|--|
    | Когда следует запускать это? | Проверка возможности отработки отказа и запуска виртуальной машины на вторичном сайте<p>Полезно для тестирования и обучения | Во время запланированного простоя и сбоев | Во время непредвиденных событий |
    ...
    | Есть ли потери данных? | None | Отсутствует. После того как отказоустойчивая реплика Hyper-V реплицирует последний набор отслеживаний изменений обратно в сервер-источник для обеспечения нулевой потери данных. | Зависит от точек восстановления и событий |
...
4. Нажмите кнопку **OK** или кнопку **Применить**.
...
-  Чтобы включить правила на изолированном сервере узла, выполните следующие действия.
    ...
    1. Запустите Брандмауэр Windows в режиме повышенной безопасности и щелкните **Правила для входящих подключений**.
...
3. На странице **Указание сервера реплики** в поле сервер реплики Введите NetBIOS-или полное доменное имя сервера реплики. Если сервер реплики входит в состав отказоустойчивого кластера, введите имя брокера реплики Hyper-V. Щелкните **Далее**.
