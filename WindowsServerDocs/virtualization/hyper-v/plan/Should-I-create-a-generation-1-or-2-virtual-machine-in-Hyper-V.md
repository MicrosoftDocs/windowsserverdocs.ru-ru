---
title: Should I create a generation 1 or 2 virtual machine in Hyper-V? (Следует ли создавать виртуальные машины 1-го и 2-го поколения в Hyper-V?).
...
ms.locfileid: "96866013"
---
# <a name="should-i-create-a-generation-1-or-2-virtual-machine-in-hyper-v"></a>Should I create a generation 1 or 2 virtual machine in Hyper-V? (Следует ли создавать виртуальные машины 1-го и 2-го поколения в Hyper-V?).
...
- Виртуальный жесткий диск, с которого требуется выполнить загрузку, не [совместим с UEFI](/previous-versions/windows/it-pro/windows-8.1-and-8/hh824898(v=win.10)).
...
|Метод загрузки|Поколение 1|2-е поколение|
|---------------|----------------|----------------|
...
- **Безопасная загрузка** Это функция, которая проверяет, подписан ли загрузчик доверенным центром сертификации в базе данных UEFI, чтобы предотвратить запуск неавторизованного встроенного по, операционных систем или драйверов UEFI во время загрузки. В виртуальных машинах поколения 2 безопасная загрузка включена по умолчанию. Если необходимо запустить операционную систему на виртуальной машине, которая не поддерживается безопасной загрузкой, ее можно отключить после создания виртуальной машины.  Дополнительные сведения см. в статье [Безопасная загрузка](/previous-versions/windows/it-pro/windows-8.1-and-8/dn486875(v=ws.11)).
...
- **Больший загрузочный том** Максимальный загрузочный том для виртуальных машин поколения 2 составляет 64 ТБ. Это максимальный размер диска, поддерживаемый. Формате. Для виртуальных машин поколения 1 максимальный загрузочный том составляет 2 ТБ для. VHDX и 2040GB для. VHD. Дополнительные сведения см. в статье [Обзор формата виртуального жесткого диска Hyper-V](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/hh831446(v=ws.11)).
...
|Контроллер гибких дисков и DMA|Контроллер гибких дисков не поддерживается|Недоступно|
...
|Шина PCI|Больше не требуется|Недоступно|
|Программируемый контроллер прерываний (PIC)|Больше не требуется|Недоступно|
|Программируемый интервальный таймер (PIT)|Больше не требуется|Недоступно|
|Устройство Super I/O|Больше не требуется|Недоступно|
...
По умолчанию виртуальные машины поколения 2 используют протокол IPv4. Чтобы использовать IPv6, выполните командлет [Set-вмфирмваре](/powershell/module/hyper-v/set-vmfirmware) Windows PowerShell. Например, следующая команда задает предпочтительный протокол для IPv6 для виртуальной машины с именем TestVM:
...
1. Отключите безопасную загрузку. Отладка ядра не совместима с безопасной загрузкой. Убедитесь, что виртуальная машина находится в отключенном состоянии, а затем используйте командлет [Set-вмфирмваре](/powershell/module/hyper-v/set-vmfirmware) . Например, следующая команда отключает безопасную загрузку виртуальной машины TestVM:

    ```powershell
    Set-VMFirmware -Vmname TestVM -EnableSecureBoot Off
    ```

2. Добавьте COM-порт. Для этого используйте командлет [Set-вмкомпорт](/powershell/module/hyper-v/set-vmcomport) . Например, следующая команда настраивает первый COM-порт на виртуальной машине TestVM для подключения к именованному каналу Тестпипе на локальном компьютере:
...
- [Планирование масштабируемости Hyper-V в Windows Server 2016](./plan-hyper-v-scalability-in-windows-server.md)
