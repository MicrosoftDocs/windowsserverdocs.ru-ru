...
> [!NOTE]
> Сведения о System Center Virtual Machine Manager (VMM) см. в разделе [Virtual Machine Manager](/system-center/vmm/overview?view=sc-vmm-2019). VMM — это продукт Майкрософт, предназначенный для управления виртуализированным центром обработки данных, который продается отдельно.
...
|Хранение|Ограничено тем, что поддерживается операционной системой узла. Оболочка Hyper-V не налагает дополнительных ограничений.|**Примечание.** Корпорация Майкрософт поддерживает сетевое хранилище (NAS) при использовании SMB 3,0. Хранилища на базе NFS не поддерживаются.|
