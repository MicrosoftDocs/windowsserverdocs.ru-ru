---
title: Обновление кластера Локальных дисковых пространств в Windows Server 2019
...
ms.locfileid: "87946327"
---
# <a name="upgrade-a-storage-spaces-direct-cluster-to-windows-server-2019"></a>Обновление кластера Локальных дисковых пространств в Windows Server 2019
...
- **Установка чистых ОС при выполнении виртуальных машин** на каждом сервере в кластере. при использовании этого параметра не происходит ПРОСТОЯ виртуальных машин, но необходимо подождать завершения заданий хранилища (зеркальное восстановление) после обновления каждого сервера, после чего потребуется настроить каждый сервер и все его приложения и роли.
...
3. Выполните следующие действия на одном сервере кластера за раз:
   ...
   5. Выполните обновление установки Windows Server 2019 на сервере, выполнив **setup.exe** и выбрав параметр "не использовать личные файлы и приложения". После завершения установки сервер остается в кластере, а служба кластеров запускается автоматически.
   ...
   9. Дождитесь завершения заданий восстановления хранилища и, чтобы все диски возвращались в работоспособное состояние. Это может занять значительное время в зависимости от количества виртуальных машин, выполняемых во время обновления сервера. Выполните следующие команды:
...
6. После обновления функционального уровня кластера используйте следующий командлет для обновления пула носителей. На этом этапе новые командлеты, такие как, `Get-ClusterPerf` будут полностью работоспособны на любом сервере в кластере.
...
3. Выполните следующие действия на одном сервере кластера за раз:
   ...
   4. Выполните чистую установку Windows Server 2019 на сервере: отформатируйте системный диск, запустите **setup.exe** и используйте параметр «Nothing». После завершения установки необходимо настроить удостоверение сервера, роли, компоненты и приложения, после чего сервер будет перезагружен.
   ...
   10. Дождитесь завершения заданий восстановления хранилища и, чтобы все диски возвращались в работоспособное состояние. Это может занять значительное время в зависимости от количества виртуальных машин, выполняемых во время обновления сервера. Выполните следующие команды:
...
3. Выполните следующие действия на одном сервере кластера за раз:
   ...
   4. Выполните обновление установки Windows Server 2019 на сервере, выполнив **setup.exe** и выбрав параметр "не использовать личные файлы и приложения".
   ...
   Это должно быть относительно быстро, так как виртуальные машины не работают. Выполните следующие команды:
...
6. После обновления функционального уровня кластера используйте следующий командлет для обновления пула носителей.
   На этом этапе новые командлеты, такие как, `Get-ClusterPerf` будут полностью работоспособны на любом сервере в кластере.
...
3. Выполните следующие действия на одном сервере кластера за раз:
   ...
   6. Выполните чистую установку Windows Server 2019 на сервере: отформатируйте системный диск, запустите **setup.exe** и используйте параметр «Nothing».
   ...
   12. Дождитесь завершения заданий восстановления хранилища и, чтобы все диски возвращались в работоспособное состояние.
       Это может занять значительное время в зависимости от количества виртуальных машин, выполняемых во время обновления сервера. Выполните следующие команды:
