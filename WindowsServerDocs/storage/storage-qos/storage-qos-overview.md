...
Представленный в Windows Server 2016 компонент качества обслуживания хранилища позволяет централизованно отслеживать и контролировать производительность хранилища виртуальных машин с помощью ролей Hyper-V и масштабируемого файлового сервера. Эта функция автоматически повышает равноправие ресурсов хранилища между несколькими виртуальными машинами с помощью одного кластера файлового сервера и позволяет настроить минимальные и максимальные цели производительности на основе политик в единицах нормализованных операций ввода-вывода.
...
|Термин|Описание|
|--------|---------------|
|Нормализованные операции ввода-вывода|Все данные об использовании хранилища измеряются в "нормализованных операциях ввода-вывода".  Это число операций ввода-вывода хранилища в секунду.  Операция ввода-вывода размером 8 КБ или меньше считается одной нормализованной операцией ввода-вывода.  Если размер такой операции превышает 8 КБ, она рассматривается как несколько нормализованных операций ввода-вывода. Например, запрос 256 КБ обрабатывается как 32 нормализованных операций ввода-вывода.<p>В Windows Server 2016 можно указать размер для нормализации операций ввода-вывода.  В кластере хранилища можно указать нормализованный размер, который будет применяться при вычислении нормализации на уровне всего кластера.  Значение по умолчанию остается 8 КБ. Этот параметр влияет на все виртуальные машины. (На виртуальных машинах, созданных на локальных томах, также затрагиваются.)|
...
|Выделенные|Тип политики, в которой Управление заданной минимальным & MaximumIOPS и пропускной способностью осуществляется для отдельных виртуальных жестких дисков или VHDx.|
...
Следующий пример команды форматируется для отображения имени виртуальной машины, имени узла Hyper-V, операций ввода-вывода и имени VHD-файла, отсортированных по операциям ввода-вывода.
...
- Состояние политики:
    ...
    - ИнсуффиЦиентсраугхпут — применяется политика, но минимальное число операций ввода-вывода не может быть доставлено.  Это может происходить, если минимальное значение для виртуальной машины или всех виртуальных машин не соответствует размеру тома хранилища.
...
Метрики производительности хранилища также собираются на уровне тома каждого хранилища в дополнение к метрикам производительности для каждого потока.  Это позволяет легко увидеть среднее общее использование в нормализованных операциях ввода-вывода, задержках и агрегатных ограничениях и резервированиях, примененных к тому.
...
Если вы создаете несколько похожих политик для разных виртуальных машин, а виртуальные машины имеют одинаковое требование к хранению, они получат аналогичную общую папку операций ввода-вывода.  Если одна виртуальная машина требует больше, а другая еще меньше, то в операциях ввода-вывода будет следовать это требование.
...
Например, при создании агрегированной политики с минимумом 300 операций ввода-вывода в секунду и максимум 500 операций ввода/вывода. Если вы примените эту политику к 5 различным VHD-файлов/VHDx, вы гарантируете, что в этих пяти файлах VHD/VHDx будет гарантировано по крайней мере 300 операций ввода-вывода в секунду (если требуется, а система хранения может обеспечить такую производительность) и не более 500 операций ввода-вывода в секунду. Если файлы VHD/VHDx имеют аналогичные высокие требования к операциям ввода-вывода в секунду и система хранения может храниться, каждый VHD-или VHDx будет получать около 100 операций ввода-вывода в секунду.

Однако если создать выделенную политику с аналогичными ограничениями и применить ее к VHD/VHDx-файлам на 5 разных виртуальных машинах, каждая виртуальная машина будет получать не менее 300 операций ввода-вывода в секунду и не более 500 операций ввода-вывода в секунду. Если виртуальные машины имеют аналогичные высокие потребности в операциях ввода-вывода и система хранения может быть работоспособна, каждая виртуальная машина будет получать около 500 операций ввода-вывода в секунду. .  Если у одной из виртуальных машин несколько VHD- или VHDX-файлов с одинаковой многоэкземплярной политикой, к ним будет применяться общее ограничение. Таким образом, общее число операций ввода-вывода в секунду для виртуальной машины из файлов с этой политикой не может превышать ограничение.
...
Используйте `Get-StorageQosFlow` командлет PowerShell, чтобы убедиться, что минимумиопс и MaximumIOPS применены к соответствующим потокам, как показано в следующем примере.
...
- **ОК** — все потоки, использующие эту политику, получают запрошенные минимумиопс.

- **ИнсуффиЦиентсраугхпут** — один или несколько потоков, использующих эту политику, не получают минимальное число операций ввода-вывода в секунду
...
Агрегированные политики могут использоваться, если требуется, чтобы несколько виртуальных жестких дисков совместно использовали один пул операций ввода-вывода в секунду и пропускную способность.  Например, если применить ту же объединенную политику к жестким дискам двух виртуальных машин, минимальное количество будет разделено между ними в соответствии с требованиями.  Оба диска будут гарантированно объединены как минимум, и они не будут превышать указанное максимальное количество операций ввода-вывода или пропускную способность.
...
Каждый виртуальный жесткий диск будет иметь значение Минимумиопс, MaximumIOPS и Максимумиобандвидс, скорректированное в зависимости от нагрузки.  Это гарантирует, что общий объем пропускной способности, используемой для группы дисков, будет в пределах диапазона, определенного политикой.  В приведенном выше примере первые два диска находятся в режиме простоя, а третья — максимально допустимое количество операций ввода-вывода в секунду.  Если первые два диска начинают выдавать операции ввода-вывода, то максимальное число операций ввода-вывода на третий диск будет снижено автоматически.

### <a name="modify-an-existing-policy"></a>Изменение имеющейся политики

Свойства Names, Минимумиопс, MaximumIOPS и Максимумиобандвидскан будут изменены после создания политики.  Однако изменить тип политики (объединенная или выделенная) нельзя.

В следующем командлете Windows PowerShell показано, как изменить свойство MaximumIOPS для существующей политики.
...
- Установите минимально определенные значения операций ввода-вывода в соответствии с политикой. перетаскивани
