...
В качестве альтернативы запуску мастера создания диска шаблона можно скопировать диск шаблона и сертификат на компьютер, на котором выполняется RSAT, и запустить программу [Protect-темплатедиск](/powershell/module/shieldedvmtemplate/protect-templatedisk?view=win10-ps) , чтобы начать процесс подписи.
