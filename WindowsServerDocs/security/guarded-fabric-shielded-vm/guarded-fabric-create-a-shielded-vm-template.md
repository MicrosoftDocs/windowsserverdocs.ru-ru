...
description: 'Дополнительные сведения: создание диска шаблона экранированной виртуальной машины Windows'
ms.topic: article
...
3. Получите или создайте сертификат для подписания VSC для VHDX, который станет диском шаблона для новых экранированных виртуальных машин. Сведения об этом сертификате будут отображаться для клиентов при создании файлов данных экранирования и авторизации дисков, которым они доверяют. Поэтому важно получить этот сертификат из центра сертификации, который будет взаимно доверенным для вас и ваших клиентов. В корпоративных сценариях, где вы одновременно являетесь узлом и клиентом, вы можете использовать этот сертификат из PKI.
...
В качестве альтернативы запуску мастера создания диска шаблона можно скопировать диск шаблона и сертификат на компьютер, на котором выполняется RSAT, и запустить программу [Protect-темплатедиск](/powershell/module/shieldedvmtemplate/protect-templatedisk) , чтобы начать процесс подписи.
