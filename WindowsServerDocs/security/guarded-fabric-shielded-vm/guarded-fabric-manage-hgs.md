---
description: Дополнительные сведения см. в статье Управление службой защиты узла.
...
HGS поставляется с [достаточной ролью администрирования](/powershell/scripting/learn/remoting/jea/overview) (JEA), встроенной в, что позволяет более безопасно управлять ею.
...
Если вы использовали `Install-HgsServer` для настройки нового леса для HGS, эти группы будут называться "Администраторы *ServiceName*" и "*ServiceName* проверяющими" соответственно, где *ServiceName* — это сетевое имя кластера HGS.
...
### <a name="disaster-recovery-planning"></a>План аварийного восстановления
...
`Export-HgsServerState`Команда выполняет резервное копирование сертификатов на основе PFX, добавленных в HGS во время выполнения команды.
...
Чтобы записать имя службы HGS, выполните команду `Get-HgsServer` и обратите внимание на неструктурированное имя в URL-адресах аттестации и защиты ключей.
...
Если вы хотите импортировать политики аттестации, доверенные для администраторов, или политики аттестации доверенного платформенного модуля, это можно сделать, `-ImportActiveDirectoryModeState` указав `-ImportTpmModeState` флаги или для [Import-хгссерверстате](https://technet.microsoft.com/library/mt652168.aspx).

Убедитесь, что перед запуском установлено последнее накопительное обновление для Windows Server 2016 `Import-HgsServerState` .
...
Чтобы проверить режим работы сервера HGS, выполните команду `Get-HgsServer` .
...
Hgs_DumpEncryption             | Требуются дампы памяти, если они включены на узле Hyper-V, которые должны быть зашифрованы с помощью ключа шифрования, доверенного для HGS. Эта политика не применяется, если на узле не включены дампы. Если эта политика и несохраненные *\_ дампы HGS* отключены, память экранированной виртуальной машины может быть сохранена в незашифрованном файле дампа.
Hgs_DumpEncryptionKey          | Отрицательная политика, чтобы убедиться, что узлы, настроенные на разрешение дампов памяти, используют определенный администратором ключ шифрования файла дампа, известный для HGS. Эта политика не применяется, если *\_ думпенкриптион HGS* отключен.
...
Чтобы проверить режим аттестации для защищенной структуры, выполните `Get-HgsServer` команду на любом узле HGS.
...
> Кроме того, вы можете пропустить проверку в это время, добавив `-SkipValidation` флаг к команде.
...
На узле-образце, который служит в качестве главного образа для доверенных компьютеров Hyper-V в вашей среде, запишите новую политику CI с помощью `New-CIPolicy` команды.
...
Если политика *HGS \_* *\_ думпенкриптион* отключена и включена политика HGS, защищенные узлы могут иметь дампы памяти (включая аварийные дампы), чтобы их можно было включить при условии, что эти дампы зашифрованы. Защищенные узлы будут передавать аттестацию только в том случае, если они либо отключают дампы памяти, либо шифруют их с помощью ключа, известного под названием HGS. По умолчанию в HGS не настраиваются ключи шифрования дампа.

Чтобы добавить ключ шифрования дампа в HGS, используйте `Add-HgsAttestationDumpPolicy` командлет, чтобы предоставить HGS с хэшем ключа шифрования дампа.
Если вы захватываете базовый модуль TPM на узле Hyper-V, настроенном с помощью шифрования дампа, хэш включается в ткглог и может быть предоставлен `Add-HgsAttestationDumpPolicy` командлету.
...
На вновь добавленном узле Hyper-V запустите `Set-HgsClientConfiguration` и укажите правильные URL-адреса для кластера HGS.
...
Если возникают проблемы и необходимо переключиться обратно в режим Active Directory, это можно сделать, запустив `Set-HgsServer -TrustActiveDirectory` .
...
Это действие требует вовлечения владельца виртуальной машины (пользователя или сущности, которой принадлежит опекун "владелец"). Для каждой экранированной виртуальной машины выполните следующие действия.

1. Выключите виртуальную машину. Невозможно снова включить виртуальную машину, пока оставшиеся шаги не будут выполнены, иначе вам потребуется запустить процесс снова.

2. Сохранить текущий предохранитель ключа в файле: `Get-VMKeyProtector -VMName 'VM001' | Out-File '.\VM001.kp'`

3. Перемещение ключевого индикатора на владельца виртуальной машины

4. Загружайте владельца обновленных сведений о защитнике из HGS и импортируйте их в свою локальную систему

5. Прочтите текущий ключевой объект в память, предоставьте новому хранителю доступ к КЛЮЧЕВому элементу и сохраните его в новый файл, выполнив следующие команды:
...
6. Скопируйте обновленный ключевой объект обратно в структуру размещения.

7. Примените ключевой объект к исходной виртуальной машине:
...
8. Наконец, запустите виртуальную машину и убедитесь, что она успешно запущена.
    ...
    > Чтобы вернуться к последнему работоспособному предохранителю ключа, выполните команду `Set-VMKeyProtector -RestoreLastKnownGoodKeyProtector`
...
11. Убедившись, что виртуальные машины по-прежнему могут начать работу с отключенными сертификатами, удалите сертификаты из HGS, выполнив следующие команды:
